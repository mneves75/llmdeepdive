...
`pnpm content:figures`), `pnpm a11y:contrast`, `pnpm budget`, `pnpm privacy`,
`pnpm audit --audit-level=high` and `pnpm render:check` (after
`pnpm exec playwright install chromium webkit firefox`). CI runs these plus a
