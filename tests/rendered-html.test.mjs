...
/**
 * Cloudflare drops a `_headers` line over 2,000 characters (Workers → Platform
 * → Limits → Static Assets: "_headers characters per line: 2,000"). The CSP is
 * one line and it GROWS with the build: gen-headers.mjs appends a sha256 per
 * distinct inline script (~55 chars) and per distinct inline style (~110, since
 * style hashes are emitted into both style-src and style-src-elem).
 *
 * Nothing else notices. The hash assertions below check each hash is *present*,
 * never that the line still fits, so going over ships the site with no CSP at
 * all while every local gate stays green.
 *
 * The 1,900 ceiling leaves ~100 characters of margin below Cloudflare's limit
...
test('the CSP header line stays inside the Cloudflare _headers line limit', () => {
  ...
  assert.ok(line, 'dist/_headers is missing Content-Security-Policy')
  assert.ok(
    line.length <= CSP_LINE_MAX,
    `CSP line is ${line.length} chars, over the ${CSP_LINE_MAX} ceiling. Cloudflare ` +
      'drops a _headers line above 2000 chars and the site then serves NO CSP, ' +
      'silently. Reduce distinct inline scripts/styles — each new inline <style> ' +
      'costs ~110 chars because its hash goes into both style-src and style-src-elem.',
  )
})
