...
covers commands, invariants, the performance gate, Cloudflare rules, and the
facts in Track 11 that must not drift.
...
prefer existing deps over hand-rolling) and **Facts that must not drift** (the
Kimi K3 numbers, which several plausible-sounding "corrections" would break).
