...
quiz:
  - question: "From lesson 7.3, why is the decode phase the natural target for graph capture while prefill usually is not?"
    options: ["Prefill has no kernels to capture","Decode repeats a short, uniform step for every token, while prefill is compute-bound with input-dependent shapes that would force constant recapture","Decode is compute-bound and prefill is memory-bound","Graph capture only works on the first request"]
    correctIndex: 1
  ...
  - question: "What does CUDA graph capture require of the memory it touches?"
    options: ["That all tensors be on the CPU","That the model be quantized first","Static addresses — the replayed graph re-executes the recorded launches against the same buffers, so tensors cannot be reallocated between steps","Nothing; addresses are resolved at replay time"]
    correctIndex: 2
  ...
  - question: "You wrap the model in torch.compile and time the very next call, which is slower than eager. What went wrong?"
    options: ["Compilation always makes models slower","The weights were corrupted by compilation","torch.compile does not support bf16","The first call includes tracing and code generation; you measured the compiler, not the compiled model"]
    correctIndex: 3
