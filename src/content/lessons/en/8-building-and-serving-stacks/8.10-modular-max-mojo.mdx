...
quiz:
  - question: "Where should a compiler-first stack most plausibly beat a hand-tuned kernel stack, and where should it most plausibly lose?"
    options: ["Win everywhere, because compilation is strictly better than hand-written code","Win only at very large batch sizes and lose at small ones","Win where launch overhead and unfused small operations dominate, such as low-batch decode; lose where a vendor kernel already saturates memory bandwidth at high batch","Win on quality of generated text and lose on throughput"]
    correctIndex: 2
  ...
  - question: "Recalling lesson 8.4, which vLLM responsibilities does a serving stack still have to provide even if a compiler handles kernel fusion?"
    options: ["Tokenization of the input string","Choosing the sampling temperature","Downloading weights from the model hub","Continuous batching and paged KV-cache allocation across concurrent requests"]
    correctIndex: 3
