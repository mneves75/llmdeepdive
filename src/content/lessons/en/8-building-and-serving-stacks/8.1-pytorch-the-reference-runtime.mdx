...
quiz:
  - question: "The classic block of lesson 4.11 pairs a token mixer with a position-wise FFN. Walking Qwen3.8-27B's 64 decoder layers, what stays the same in every one of them?"
    options: ["Nothing — every layer has a different shape","The two RMSNorms and the gated FFN; only the token mixer alternates between attention and Gated DeltaNet","The token mixer; the FFN alternates","Only the residual connection"]
    correctIndex: 1
  ...
  - question: "In the attention layers, what is the output width of the q_proj weight?"
    options: ["6144, because 24 query heads at head dimension 256 give 24 × 256","5120, because projections are always square","12288, because the layers are gated: 24 × 256 for queries plus the same again for the gate","17408, the FFN intermediate size"]
    correctIndex: 2
  ...
  - question: "Why is eager PyTorch a poor production server for this model even though it is the reference?"
    options: ["It computes different mathematics from the serving engines","It cannot load bf16 weights","It only runs on CPU","It dispatches every operation separately, allocates per request, and has no paged KV or continuous batching, so it is launch-bound and memory-wasteful"]
    correctIndex: 3
