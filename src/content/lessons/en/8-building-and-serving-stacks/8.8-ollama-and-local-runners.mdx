...
quiz:
  - question: "On a sub-24 GiB deployment, where Ollama currently defaults to a 4K context, you paste a 40,000-token document. The reply ignores the beginning. What is the most likely cause?"
    options: ["The Gated DeltaNet layers forget early tokens by construction","GGUF cannot store prompts longer than 32,768 tokens","The effective num_ctx is 4K, so the prompt was truncated from the front before the model saw it","The model card's 262,144 context only applies to the bf16 checkpoint"]
    correctIndex: 2
    explanation: "On this stated VRAM tier, the current default is 4K. Inspect the effective num_ctx: the native 262,144 context is available only if the runtime supports it, you raise the limit, and you pay the KV cost from lesson 7.2."
  - question: "Using lesson 8.7, what does a runner actually pull when you request a 4-bit build of this model?"
    options: ["A safetensors checkpoint the runner quantizes at load time","A compressed archive of PyTorch weights expanded into RAM","A CUDA engine compiled for the local GPU","A GGUF container whose manifest names the architecture and gives each tensor its own quantization type"]
    correctIndex: 3
