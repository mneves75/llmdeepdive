...
quiz:
  - question: "Why must volatile content such as a timestamp go at the end of a prompt rather than the beginning?"
    options: ["Timestamps confuse the tokenizer","The model reads prompts backwards","Late tokens are cached but early ones are not","Prefix matching is exact and anchored at position one, so a differing early token means no blocks are shared at all"]
    correctIndex: 3
  ...
  - question: "What does grammar-constrained decoding guarantee, and what does it not?"
    options: ["It guarantees factual accuracy","It guarantees output conforms to the schema, not that the content is correct or well-reasoned","It guarantees lower latency","It guarantees the model used thinking mode"]
    correctIndex: 1
