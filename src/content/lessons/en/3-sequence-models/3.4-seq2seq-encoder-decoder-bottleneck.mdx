...
teachBack:
  prompt: "Describe classic recurrent encoder-decoder training and inference, then explain the two distinct failures the fixed-vector interface causes, why widening only partly relieves capacity, and why it does not repair the gradient path or the one-summary interface."
...
quiz:
  - question: "Lesson 3.3 showed that an LSTM's long-range signal decays as a product of forget gates. In a seq2seq model with a 40-token source and a 45-token target, how long is that product for the earliest source token?"
    options: ["45 steps, the decoder only", "About 85 steps — the whole encoder plus the whole decoder, since the only route runs through the final encoder state", "One step, because the encoder state is passed directly", "It does not depend on the lengths"]
    correctIndex: 1
  ...
  - question: "Why is the fixed-vector bottleneck not solved by making the vector much wider?"
    options: ["Wider vectors cannot be trained", "The decoder ignores the encoder state entirely", "The interface still requires one summary, fixed before decoding starts, to serve every target step regardless of what each step needs", "Cross-entropy requires equal source and target lengths"]
    correctIndex: 2
  ...
  - question: "What conditions the first decoder step in classic seq2seq?"
    options: ["Future target tokens", "A random test label", "The source length only", "The encoder's final representation together with a start token"]
    correctIndex: 3
