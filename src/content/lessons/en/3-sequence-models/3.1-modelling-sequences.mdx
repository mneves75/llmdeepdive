...
quiz:
  ...
  - question: "Why does a fixed-size state impose a hard ceiling rather than merely a difficulty?"
    options: ["Fixed states cannot be trained with backpropagation", "If a task requires more bits to be carried forward than the state can store, no amount of training can make it fit", "Fixed states cannot represent negative numbers", "The ceiling applies only to short sequences"]
    correctIndex: 1
  ...
  - question: "What distinguishes a causal sequence model?"
    options: ["It uses no parameters", "Every sequence has equal length", "The representation at position t may not depend on any position after t", "It predicts exactly one class"]
    correctIndex: 2
