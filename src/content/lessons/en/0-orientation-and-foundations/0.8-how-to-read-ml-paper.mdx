...
quiz:
  - question: "The Qwen3.8-27B model card reports 89.2 on GPQA Diamond. Which write-up is defensible?"
    options: ["89.2 on GPQA Diamond means it answers 89.2 percent of science questions correctly in practice","Qwen3.8-27B achieves 89.2 on GPQA Diamond, proving graduate-level science ability","The vendor reports 89.2 on GPQA Diamond; the harness and sampling settings are unspecified and no independent reproduction is available","Qwen3.8-27B is the strongest open model at scientific reasoning"]
    correctIndex: 2
  ...
  - question: "There is no technical report for Qwen3.8-27B. What follows about its training data?"
    options: ["It can be recovered by reading the config file","It is presumably the same as the previous generation's","It can be inferred from the benchmark scores","Nothing can be honestly claimed about it, because no such information was published"]
    correctIndex: 3
  ...
  - question: "Lesson 0.7 loaded the model configuration to read hidden_size, num_hidden_layers, and vocab_size. Where do those numbers sit on the evidence hierarchy?"
    options: ["They are primary and directly verifiable for a pinned artifact revision, which should travel with the claim","They cannot be trusted without a technical report","They are vendor claims and need the same hedging as benchmark scores","They are secondary, since the config is generated from the weights"]
    correctIndex: 0
    explanation: "The config ships with the weights and describes the tensors that must exist for the model to load, so it can be checked against the weight index. That is stronger evidence than a self-reported score, but it remains a versioned fact about a particular artifact."
citations:
...
:::insight
Notice that the three passes have produced three *different grades* of evidence from one release, and the useful discipline is to keep them separated in your own writing. Config-verified shapes are primary and need a revision, not a capability hedge. Card-only capability claims are vendor-reported and need attribution in the same sentence. Unpublished training details are absent, and absence is a finding to report, not a gap to fill.
:::
