...
quiz:
  ...
  - question: "What does the gradient of a scalar loss point toward?"
    options: ["The global minimum, reachable in one step","The direction of steepest local increase","The largest parameter value","A randomly chosen parameter"]
    correctIndex: 1
  ...
  - question: "Lesson 0.5 showed that two stacked linear layers collapse into one matrix. What does that imply about their gradients?"
    options: ["Gradients cannot be computed through a product of matrices","The chain rule does not apply to linear maps","The whole stack has the expressive power of one layer, so extra depth adds parameters and cost without adding functions the network can represent","Each layer receives an identical gradient"]
    correctIndex: 2
