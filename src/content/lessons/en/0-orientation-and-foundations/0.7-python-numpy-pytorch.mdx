...
quiz:
  - question: "Reading Qwen3.8-27B's configuration gives vocab_size 248320. What does that number describe exactly?"
    options: ["The number of tokens the model can process at once","The number of parameters in the output layer","The number of distinct words the model knows","The number of rows in the embedding matrix, which may be padded above the tokenizer's live vocabulary"]
    correctIndex: 3
  ...
  - question: "Why is optimizer.zero_grad normally called before backward?"
    options: ["PyTorch accumulates gradients into the grad fields rather than replacing them","It moves tensors to the GPU","It selects the loss function","It deletes the model parameters"]
    correctIndex: 0
  ...
  - question: "Lesson 0.6 showed that a learning rate of 1.1 on L(w) = w squared makes the parameter diverge. If you saw that behavior in a real training loop, which of these would autograd have told you?"
    options: ["It would automatically reduce the learning rate","Nothing; autograd computes correct gradients for a diverging run exactly as it does for a converging one","It would report the gradient as not-a-number immediately","It would raise an exception on the second step"]
    correctIndex: 1
...
:::caveat
Reading a number is not the same as understanding what it counts. The value 248,320 is the number of embedding rows. The published tokenizer maps 248,077 ids, leaving 243 tensor rows without a tokenizer mapping. Divisibility by 512 is hardware-convenient, but the artifacts do not document why this exact shape was chosen, what signal those unmapped rows received, or how every runtime treats them. Take the configuration as authority on tensor shapes and the tokenizer as authority on text-to-id mapping; lesson 1.2 shows why conflating them produces a plausible and wrong claim.
:::
