...
summary: "A language model maps context to next-token scores and probabilities; Qwen3.8-27B has 248,320 output rows, while its published tokenizer maps 248,077 ids."
tier: "foundations"
locale: "en"
prerequisites: []
...
quiz:
  - question: "How many numbers does one forward pass of Qwen3.8-27B produce for a single position, before decoding?"
    options: ["One, the chosen token","248,320, one per output-tensor row","27 billion, one per parameter","5120, one per hidden dimension"]
    correctIndex: 1
    explanation: "The model's output tensor has 248,320 rows, so it produces that many scores per position. Its published tokenizer maps 248,077 ids; a runtime must decide how to treat the remaining 243 logits before selecting a mapped token."
  - question: "Why can repeated next-token prediction produce a coherent paragraph?"
    options: ["Tokens already contain full paragraphs","The model stores every possible paragraph","Each selected token is appended, so the next prediction conditions on a longer context","The tokenizer checks factual accuracy"]
    correctIndex: 2
  ...
  - question: "A model assigns 0.93 probability to a factually false continuation. What does that number mean?"
    options: ["The claim was verified during training","The claim is 93 percent true","The model retrieved it from a trusted source","The model considers that continuation highly likely text, which is not a claim about the world"]
    correctIndex: 3
...
:::caveat
Take any specific probability quoted in this lesson as illustrative. The output width of 248,320 is a published fact about Qwen3.8-27B; the exact mass it puts on any particular token is not, and we never invent measurements in this course.
:::
