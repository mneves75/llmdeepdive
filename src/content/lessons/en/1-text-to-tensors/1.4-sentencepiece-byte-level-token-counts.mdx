...
quiz:
  ...
  - question: "What is the only reliable way to know whether a text fits a model's context window?"
    options: ["Measure the byte size of the text","Assume four characters per token","Count the words and divide by 0.75","Run the exact deployed tokenizer over the fully assembled request, including chat template and special tokens"]
    correctIndex: 3
  ...
  - question: "Lesson 1.3 showed that BPE merges are learned from a corpus in frequency order. Why does that explain uneven token counts across languages?"
    options: ["Pairs frequent in the training corpus became single pieces, so text unlike that corpus is assembled from more fragments","Because non-English text is stored in a different encoding","Because languages have different alphabets","Because longer words always cost more tokens"]
    correctIndex: 0
...
citations:
  ...
  - title: "Qwen3.8-27B on Cloudflare Workers AI"
    authors: "Cloudflare"
    year: 2026
