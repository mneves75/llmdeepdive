...
quiz:
  - question: "In the standard analogy benchmark, computing king minus man plus woman and searching for the nearest word excludes the three query words from the candidate set. Why does that matter?"
    options: ["Published re-evaluation shows that keeping query words among the candidates can sharply reduce accuracy and drive several categories to zero","It guarantees the answer is grammatically correct","It prevents the vectors from being normalized","It makes the computation faster"]
    correctIndex: 0
    explanation: "Candidate exclusion materially changes the measured result. Linzen reported drastic drops without it, including zero accuracy in several categories; reporting the rule is therefore part of the result."
  - question: "What does skip-gram train a word representation to help predict?"
    options: ["The document's title","Nearby context words","A dictionary definition","The word's spelling"]
    correctIndex: 1
  ...
  - question: "Lesson 1.5 noted that Qwen3.8-27B does not tie its input and output embeddings. Which older idea does that echo?"
    options: ["GloVe used two co-occurrence matrices","n-gram models stored two count tables","word2vec also maintained separate input and output embedding tables, because reading a word and predicting it are different jobs","Nothing; untied embeddings are a recent invention with no precedent"]
    correctIndex: 2
