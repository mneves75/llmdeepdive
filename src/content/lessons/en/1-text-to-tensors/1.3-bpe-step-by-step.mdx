...
quiz:
  - question: "In the corpus where low appears 5 times, lower twice, and lowest three times, the first merge joins l and o. What is the count of the winning pair on the second round?"
    options: ["3, for the pair e followed by s","10, for the pair l followed by o again","5, for the pair lo followed by w","10, for the pair lo followed by w"]
    correctIndex: 3
  ...
  - question: "Does tokenizing a user's prompt learn new merges?"
    options: ["No, it applies the fixed learned merges in their learned order","Only during fine-tuning","Only when the prompt contains punctuation","Yes, for every new sentence"]
    correctIndex: 0
  ...
  - question: "Lesson 1.2 said Qwen3.8-27B's tokenizer is byte-level. What does that guarantee about BPE's base alphabet here?"
    options: ["The base alphabet is learned along with the merges","The byte-derived base represents every UTF-8 byte of valid normalized Unicode text","There is no base alphabet, only merged pieces","The base units are English letters, so other scripts need an unknown token"]
    correctIndex: 1
    explanation: "The published ByteLevel+BPE path needs no unknown token for valid Unicode text after normalization; unusual strings may still expand into many byte-derived pieces."
citations:
