...
quiz:
  - question: "Qwen3.8-27B has 248,320 tensor rows and hidden size 5120, and does not tie its embeddings. How many parameters sit in the input embedding and output projection combined?"
    options: ["About 1.271 billion","About 2.54 billion","About 27 billion","About 5120"]
    correctIndex: 1
  ...
  - question: "Why can two tokens end up close together in embedding space?"
    options: ["They contain the same number of letters","Their ids are numerically adjacent","Training found their predictive uses related","They always mean exactly the same thing"]
    correctIndex: 2
  ...
  - question: "Lesson 1.4 showed that Portuguese text can need more tokens than its English equivalent. What does that imply about embedding lookups for that text?"
    options: ["The embeddings are combined into a single vector first","The rows fetched are lower quality","The embedding table must be larger","More rows are fetched and more positions enter the network, so the same content costs more work at every layer"]
    correctIndex: 3
...
:::insight
The output matrix is the same object lesson 0.1 described from the other side. Producing 248,320 raw logits means taking the final 5120-dimensional state and computing a dot product against each output row; the tokenizer maps 248,077 ids, and runtime handling of the other 243 logits is not specified by these artifacts. Untying lets the model learn one geometry for reading a token and a different geometry for predicting it, which are genuinely different jobs: what makes two tokens interchangeable as *input* is not the same as what makes them competitors as *output*.
:::
