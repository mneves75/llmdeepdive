...
teachBack:
  prompt: "Distinguish GPTQ, AWQ, GGUF, and QAT by role, explain why the pinned Q4_K_M GGUF of Qwen3.8-27B is 17.1 GB rather than the 13.5 GB pure-code floor, and say why converting between lossy formats cannot restore quality."
...
quiz:
  ...
  - question: "Using lesson 7.9, why can a smaller group size improve a 4-bit quantization's quality?"
    options: ["Smaller groups remove the need for calibration data", "Smaller groups increase every weight's bit width", "Group size only affects file names", "Each group gets its own scale, so one outlier no longer coarsens the step size for a whole tensor"]
    correctIndex: 3
...
citations:
  ...
  - title: "Pinned Qwen3.8-27B Q4_K_M GGUF artifact"
    authors: "Unsloth"
    year: 2026
