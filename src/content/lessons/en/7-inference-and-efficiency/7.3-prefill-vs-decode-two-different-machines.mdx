...
quiz:
  - question: "Which user-facing metric does prefill most directly control?"
    options: ["Inter-token latency", "Tokenizer vocabulary size", "Weight download time", "Time to first token, since it spans prompt processing before any output exists"]
    correctIndex: 3
  ...
  - question: "Why does decode memory traffic in 48 of Qwen3.8-27B's 64 layers stay flat as the context grows?"
    ...
    correctIndex: 0
    explanation: "The reference float32 DeltaNet matrix state is 3 MiB per layer, 144 MiB total, independent of sequence length; only the 16 attention layers read a growing cache."
citations:
