...
The first gate is arithmetic, not software. The weights cost 54 GB in bf16 (lessons 7.2 and 9.3 own that math). On one 80 GB GPU that leaves $80 - 54 = 26$ GB, about 24 GiB, of KV pool — at 64 KiB per token, $24\ \text{GiB} \div 64\ \text{KiB} \approx 390{,}000$ tokens of pooled attention context, enough for roughly a dozen 32K sessions as lesson 7.8 computed. If your hardware budget is a 24 or 48 GB card, this whole lesson is premature: you are on the quantized-artifact path through lesson 7.12 and the local runners of Track 8, not choosing among data-center engines.
