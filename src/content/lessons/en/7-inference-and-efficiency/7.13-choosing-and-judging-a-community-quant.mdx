...
quiz:
  - question: "A GGUF artifact is labelled Q4_K_M and measures 4.8944 bits per weight. What accounts for the gap between 4 and 4.8944?"
    ...
    correctIndex: 0
    explanation: "Bits per weight averages the whole checkpoint, metadata included, which is exactly why it exceeds the nominal width. Lessons 7.9 and 8.7 apply the same reasoning to Qwen3.8-27B and land on a different rate — the effect is general, the number is per artifact."
  - question: "Using lesson 7.3, why does quantizing raise text-generation speed substantially while doing nothing for prompt processing?"
    options: ["Decode is memory-bandwidth-bound and moves the weight bytes per token, while prefill is compute-bound — and unpacking quantized weights adds arithmetic there rather than removing it", "Prefill skips the feed-forward blocks entirely", "Quantized kernels are only implemented for the decode path in every runtime", "Prompt processing reads the weights from disk rather than from memory"]
    correctIndex: 0
    explanation: "Lesson 7.3 separated the two machines: decode re-reads roughly the whole weight set per generated token, so fewer stored bytes directly attack its bottleneck. Prefill already saturates the arithmetic units, and dequantizing is extra work for them, which is why the measured table shows prompt processing not merely flat but slightly worse than F16."
