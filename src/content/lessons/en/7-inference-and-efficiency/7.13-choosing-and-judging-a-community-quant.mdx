...
quiz:
  - question: "A GGUF artifact is labelled Q4_K_M and measures 4.8944 bits per weight. What accounts for the gap between 4 and 4.8944?"
    options: ["The file stores each 4-bit code padded to a full byte", "Block scales, super-block metadata, and tensors the recipe deliberately keeps at higher precision are all counted in the average", "The tokenizer and chat template are counted as weights", "The measurement includes the KV cache at the default context length"]
    correctIndex: 1
  ...
  - question: "Using lesson 7.3, why does quantizing raise text-generation speed substantially while doing nothing for prompt processing?"
    options: ["Prefill skips the feed-forward blocks entirely", "Quantized kernels are only implemented for the decode path in every runtime", "Decode is memory-bandwidth-bound and moves the weight bytes per token, while prefill is compute-bound — and unpacking quantized weights adds arithmetic there rather than removing it", "Prompt processing reads the weights from disk rather than from memory"]
    correctIndex: 2
  ...
  - question: "What does an importance matrix contribute to a quantization run?"
    options: ["A direct measurement of how much each individual weight changes the model's output", "A guarantee that the resulting file uses the IQ format", "A compressed copy of the original weights used to correct errors at inference time", "Per-column activation statistics gathered from calibration data, which weight the quantizer's error so precision goes where that corpus exercised the model hardest"]
    correctIndex: 3
...
:::note
Bits per weight is model-dependent. It shifts with tensor inventory, embedding size, protected tensors, and metadata. The 4.8944 above is a published measurement for this Llama-3.1-8B artifact; lesson 7.10 deliberately does not infer an exact Qwen bpw from a rounded 27B label. Read bpw as a fact about one artifact, never as a property of a format.
:::
