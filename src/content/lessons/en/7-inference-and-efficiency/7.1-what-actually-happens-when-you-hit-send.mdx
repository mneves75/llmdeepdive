...
quiz:
  - question: "Why can prefill process every prompt position in a single parallel pass?"
    options: ["The prompt contains no attention", "Causal self-attention lets each position compute its output from already-known earlier positions simultaneously", "The model skips its layers during prefill", "Prompt tokens require no projections"]
    correctIndex: 1
  ...
  - question: "Without speculative decoding or MTP, a 500-token reply requires about how many target-model decode passes?"
    options: ["Exactly one", "One per layer, so 64", "About 500 — one per generated token", "None; prefill emits the whole reply"]
    correctIndex: 2
    explanation: "Baseline autoregressive decoding finalizes one token per target pass. Speculative and multi-token methods can amortize one target pass across several accepted tokens."
citations:
