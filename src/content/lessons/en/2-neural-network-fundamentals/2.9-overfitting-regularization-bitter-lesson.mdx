...
summary: "Generalization is proven on held-out evidence, not on training fit — and dataset scale, diversity, and deduplication are empirical regularization choices, not universal substitutes for evaluation."
tier: "foundations"
locale: "en"
...
quiz:
  - question: "Lesson 2.8 introduced normalization and residual connections. Are they regularizers?"
    options: ["Yes, they are the main defense against overfitting", "Not primarily — they exist so deep composition stays numerically trainable, though they can have incidental effects on generalization", "Yes, they replace weight decay entirely", "No, they have no effect on training at all"]
    correctIndex: 1
  ...
  - question: "Why should the final test set not guide repeated hyperparameter choices?"
    options: ["Test examples have no labels", "Optimizers cannot read them", "Repeated tuning leaks information into the selection process, so the reported number no longer estimates performance on unseen data", "It changes the token IDs"]
    correctIndex: 2
  ...
  - question: "A vendor publishes a model card with benchmark scores but no technical report. What can you legitimately say about its training data?"
    options: ["That it was trained on roughly ten trillion tokens", "That the data must have been mostly web text", "That the cutoff can be read off the release date", "Nothing — dataset composition, token counts, and cutoff are unpublished, and inferring them from benchmark scores is speculation"]
    correctIndex: 3
...
:::insight
This is the practical face of the **bitter lesson**. Sutton's historical argument is that AI methods leveraging growing computation through general learning and search have repeatedly overtaken systems built from elaborate hand-coded domain structure. This observation does not establish that compute or data scale replaces regularization; that tradeoff remains an empirical property of a particular training regime. Read the lesson carefully, though. It is a claim about where effort has historically paid off, not a theorem that larger always wins, and it is not permission to ignore algorithmic efficiency, data governance, evaluation, or safety — every one of which becomes more consequential, not less, as compute magnifies the effect of your objective and your corpus.
:::
