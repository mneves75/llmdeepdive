...
with a learned per-feature gain $g$ and a small stabiliser $\varepsilon$, conventionally around $10^{-6}$. Walk one vector through. Say a token's hidden state at layer 40 has a root-mean-square value of 12.0 across its 5120 entries. The denominator is $\sqrt{144+10^{-6}}=12.0$ to any precision that matters, so every entry is divided by 12 and the sublayer sees a vector of unit RMS. Now consider a near-dead position whose entries are all around $10^{-5}$: the mean square is $10^{-10}$, which is ten thousand times smaller than $\varepsilon$, so the epsilon dominates the square root and the output is scaled down toward zero rather than exploding through a division by almost nothing. That is precisely the job epsilon does — it is not a rounding fudge, it is the guard that keeps the operation defined at the bottom of the range.
