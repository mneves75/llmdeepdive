...
quiz:
  - question: "Lesson 2.7 mentioned that normalization scale parameters are usually placed in a no-decay optimizer group. Why does that make sense?"
    options: ["They have no gradients", "They are integers", "They set the operating scale of the normalized signal rather than acting as feature weights, so shrinking them toward zero systematically suppresses the layer's output", "Weight decay is undefined for them"]
    correctIndex: 2
  ...
  - question: "In a pre-norm residual block, what does the normalization actually normalize?"
    options: ["The residual stream, which is therefore kept at unit scale throughout the model", "The gradients only", "The model's output logits", "The copy of the hidden state handed to the sublayer — the residual stream itself continues unnormalized"]
    correctIndex: 3
