...
quiz:
  ...
  - question: "In Qwen3.8-27B, what does the feed-forward block do to a token's hidden vector?"
    options: ["Mixes information between different token positions", "Expands it from 5120 to an intermediate width of 17408, applies a gated activation, and projects back to 5120", "Converts it into token IDs", "Normalizes it across the batch"]
    correctIndex: 1
  ...
  - question: "What is the role of hidden width?"
    options: ["It sets the dataset size", "It chooses the tokenizer", "It controls how many intermediate features a stage can represent", "It guarantees generalization"]
    correctIndex: 2
...
Look at what happened geometrically. In the new $(h_1,h_2)$ coordinates the four points sit at $(0,0)$, $(1,0)$, $(1,0)$, and $(2,1)$. The two different-input, XOR-positive corners collapse to the same hidden point $(1,0)$, while the two equal-input, XOR-negative corners land at $(0,0)$ and $(2,1)$. One straight boundary now separates the shared positive point from both negative points. The output unit is still a perceptron, unchanged from lesson 2.1. The hidden layer changed what it is looking at.
