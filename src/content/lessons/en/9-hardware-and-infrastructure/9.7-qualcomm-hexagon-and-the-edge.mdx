...
quiz:
  - question: "Using the quantization ladder from lesson 7.9, what does pushing Qwen3.8-27B from 4-bit to 2-bit actually buy on a handset?"
    options: ["A naive 6.75 GB of packed codes instead of the pinned 17.1 GB Q4_K_M artifact — still far over an app-sized budget, at a severe quality cost","A fit inside a 3 GB budget with acceptable quality","No size change, since metadata dominates","A fourfold speedup with no memory change"]
    correctIndex: 0
  ...
  - question: "Lesson 6.11 described distillation as training a small model against a large one's behaviour. Why is that the load-bearing technique for edge deployment rather than a nice-to-have?"
    options: ["It compresses the same 27B weights further than 4-bit formats can","It changes the parameter count, which is the term quantization cannot reduce","It removes the need for an NPU toolchain","It eliminates the KV cache"]
    correctIndex: 1
    explanation: "Quantization reduces bytes per parameter and can go below four bits; only a genuinely smaller network changes the parameter count itself, which is what a phone-sized budget requires."
citations:
