...
quiz:
  - question: "Applying lesson 9.3's budget arithmetic, why does a rented GPU's advertised throughput not translate directly into a low cost per token?"
    options: ["Because throughput figures always exclude the KV cache","Because vLLM charges a per-token licence fee","You rent seconds, not tokens, so the effective price is the fixed rental divided by the tokens you actually generate — utilization decides","Because bf16 weights cost more to read than 4-bit weights"]
    correctIndex: 2
  ...
  - question: "Lesson 9.3 sized an 80 GB accelerator as 54 GB of weights plus 64 KiB per cached token. Why does that arithmetic belong in a cost model at all?"
    options: ["Because memory is billed separately from compute","Because it determines the per-million input price","Because it sets the electricity draw","Because it caps concurrent sequences, which caps achievable throughput, which is the denominator of cost per token"]
    correctIndex: 3
