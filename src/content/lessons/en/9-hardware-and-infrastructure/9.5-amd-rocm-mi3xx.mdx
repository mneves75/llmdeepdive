...
quiz:
  ...
  - question: "A team wants the fastest possible single-user chat and considers one 192 GB part at a vendor-reported 5.3 TB/s against two 80 GB parts at 3.35 TB/s each in tensor parallel. Which reasoning is correct?"
    options: ["The single part is always faster, because sharding adds latency","They are identical, because total bandwidth is what matters","Neither can exceed 60 tokens per second","The pair can be faster, because each device reads only about 27 GB per step, roughly 8 milliseconds, though an all-reduce per layer eats into the gain"]
    correctIndex: 3
...
$$
\frac{136{,}909\ \text{MiB}}{16{,}528\ \text{MiB}} \approx 8\ \text{full-context sequences},
$$
...
$$
t_{\text{checkpoint estimate}} \approx\ \frac{54\times10^{9}\ \text{B}}{5.3\times10^{12}\ \text{B/s}} \approx\ 10.2\ \text{ms} \quad\Longrightarrow\quad \approx 98\ \text{tokens per second},
$$
