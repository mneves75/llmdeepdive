...
quiz:
  - question: "Lesson 5.8 described tensor parallelism as splitting weight matrices across devices. For Qwen3.8-27B, which dimension caps the clean tensor-parallel degree at 4?"
    options: ["The 5120 hidden size","The 17408 FFN intermediate size","The 4 KV heads of grouped-query attention","The 248,320 padded vocabulary"]
    correctIndex: 2
  ...
  - question: "Why does an all-reduce payload of about 1.9 MiB per token still limit throughput on a slow interconnect?"
    options: ["Because 1.9 MiB exceeds PCIe bandwidth","Because the payload must be sent to every node in the cluster","Because all-reduce requires the full KV cache to be transferred","Because the cost is dominated by the latency of 128 serial synchronizations per token, not by the bytes"]
    correctIndex: 3
