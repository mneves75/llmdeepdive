...
quiz:
  - question: "What makes SFT different from pretraining in practice?"
    options: ["A different definition of softmax", "No tokenization", "Curated instruction-response data and a loss focused on desired outputs", "Weights are never updated"]
    correctIndex: 2
  ...
  - question: "Lesson 6.1 argued that a base model already possesses the capability it seems to lack. What follows for SFT data design?"
    options: ["Only very large SFT sets can install any behaviour", "SFT must reteach grammar and world knowledge from scratch", "The base checkpoint must be discarded before SFT", "Volume of capability examples is the wrong lever; the demonstrations must show which behaviour to select by default"]
    correctIndex: 3
