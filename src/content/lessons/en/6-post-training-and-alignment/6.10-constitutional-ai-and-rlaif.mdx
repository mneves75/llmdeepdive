...
quiz:
  - question: "What does the constitution provide?"
    options: ["A mathematical proof that the model is harmless", "A replacement tokenizer", "Explicit principles used to critique, revise, or rank behaviour", "Unlimited context"]
    correctIndex: 2
  ...
  - question: "Lesson 6.4 warned that a reward model inherits the confounders of the pipeline that produced its labels. What is the corresponding risk when the labels come from a model judge?"
    options: ["AI labels are noisier than human labels in every dimension", "Model judges cannot produce pairwise comparisons", "The reward model no longer needs a rubric", "The judge's own blind spots are inherited systematically rather than randomly, so errors correlate instead of averaging out"]
    correctIndex: 3
