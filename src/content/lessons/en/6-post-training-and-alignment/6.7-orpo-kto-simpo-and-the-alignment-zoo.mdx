...
quiz:
  - question: "Which method is specifically designed to use unpaired desirable or undesirable feedback?"
    options: ["Only PPO", "Beam search", "FlashAttention", "KTO"]
    correctIndex: 3
  ...
  - question: "Lesson 6.6 showed that DPO's frozen reference anchors the objective. What changes when SimPO removes it?"
    options: ["The explicit reference anchor disappears; length normalization, reward scale, and target margin shape the loss but do not guarantee proximity to the original policy", "The method can no longer use preference pairs", "Response log-probabilities can no longer be computed", "The chosen response's likelihood is guaranteed to rise"]
    correctIndex: 0
    explanation: "The reference term was doing explicit regularization work, not bookkeeping. SimPO has a reward scale and target margin, but neither is a hard bound on divergence from the starting policy."
  - question: "What is a defining feature of SimPO?"
    options: ["A required separate reward model", "A reference-free, length-normalized policy score with a target margin", "No preference signal", "Updating only the tokenizer"]
    correctIndex: 1
...
The pairwise loss trains the chosen reward to exceed the rejected reward by a target margin $\gamma$. The scale $\beta$ controls the strength of reward differences; neither parameter is a reference-policy constraint. Work through what length normalization does with $\beta=1$, because it is not a cosmetic adjustment. Take two responses to the same prompt. Response A is 40 tokens with a summed log-probability of $-60$. Response B is 200 tokens with a summed log-probability of $-240$. Under summed scoring, A scores $-60$ and B scores $-240$, so A wins decisively. Under SimPO's normalization, A averages $-1.5$ per token and B averages $-1.2$, so B wins. Same two responses, same model, opposite ranking — produced entirely by the choice of reduction. Neither is "correct"; they encode different beliefs about whether a long response should be penalized for its length. Which is why the interaction with truncation policy and desired verbosity has to be evaluated on your data rather than assumed from a paper.
