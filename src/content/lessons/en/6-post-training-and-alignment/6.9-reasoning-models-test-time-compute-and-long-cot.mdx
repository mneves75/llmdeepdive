...
summary: "Qwen3.8-27B ships test-time compute as a product feature: thinking mode is on by default, reasoning effort changes the effective prompt and budget pressure, and the off switch is often the correct route."
tier: "advanced"
locale: "en"
...
teachBack:
  prompt: "Explain why Qwen3.8-27B samples its thinking segment at temperature 1.0 but its instruct answers at 0.7, and why a reasoning label is not portable evidence by itself."
...
quiz:
  - question: "When does more test-time compute most plausibly help?"
    options: ["Whenever it repeats the same answer verbatim", "When the system can generate meaningfully different candidates and select or verify them", "Only when temperature is zero", "When no task signal exists"]
    correctIndex: 1
