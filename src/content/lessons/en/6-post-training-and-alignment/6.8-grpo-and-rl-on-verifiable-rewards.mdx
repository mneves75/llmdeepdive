...
quiz:
  ...
  - question: "Lesson 6.4 catalogued the ways a learned reward model can be gamed. Which of those failure modes does a unit-test checker actually remove?"
    options: ["All of them, because programmatic checks cannot be exploited", "Persuasion by surface features — length, confident tone, formatting — since a test suite is indifferent to prose", "Distribution shift, since tests generalize to unseen problems", "Annotator disagreement, since tests eliminate the need for evaluation"]
    correctIndex: 1
  ...
  - question: "Which is a verifiable reward?"
    options: ["Whether prose sounds confident to the policy itself", "The number of adjectives", "Whether submitted code passes isolated tests", "A random scalar"]
    correctIndex: 2
