...
quiz:
  - question: "Why can a capable base model continue an instruction instead of obeying it?"
    options: ["Its tokenizer cannot represent commands", "Its objective learned likely text continuations, not a privileged assistant role", "Self-attention works only on documents", "It has no learned language knowledge"]
    correctIndex: 1
  ...
  - question: "In a self-attention layer, which projections does post-training change to install assistant behaviour?"
    options: ["New role-specific attention heads are grown for system, user, and assistant", "The softmax is replaced by a rule-based router", "The same query, key, value, and output projections that already existed — their values move, no new mechanism appears", "Attention is disabled and replaced by a lookup table of approved answers"]
    correctIndex: 2
  ...
  - question: "What does post-training primarily change?"
    options: ["The definition of matrix multiplication", "The physical size of every token", "The internet available during pretraining", "The distribution of behaviours the model selects for prompts"]
    correctIndex: 3
