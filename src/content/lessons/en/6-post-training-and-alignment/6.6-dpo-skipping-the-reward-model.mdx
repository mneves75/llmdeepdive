...
quiz:
  - question: "What role does the reference model play in standard DPO?"
    options: ["It generates the tokenizer alphabet", "It supplies human labels automatically", "It anchors relative log-probability changes for chosen and rejected responses", "It removes the need for preference data"]
    correctIndex: 2
  ...
  - question: "Lesson 6.4 described length as the canonical confounder in preference data. Why is offline DPO especially exposed to it?"
    options: ["It cannot process responses longer than the context window", "It normalizes every response to the same length automatically", "It discards the rejected response before computing the loss", "It scores whole responses by summed token log-probability, so length differences enter the objective directly and there is no fresh sampling to reveal the drift"]
    correctIndex: 3
