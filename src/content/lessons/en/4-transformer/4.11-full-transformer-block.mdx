...
quiz:
  - question: "In Qwen3.8-27B's attention block, how wide is the query state that enters attention, and why?"
    options: ["5120, because projections in a Transformer are always square", "1024, the same as the key projection", "248320, the vocabulary size", "6144, because 24 heads × head dimension 256 need not equal the hidden size 5120 — half of a 12288-wide gated projection"]
    correctIndex: 3
  ...
  - question: "Using the previous lesson's pre-norm pattern, which expression describes the first half of a Qwen block?"
    options: ["x1 = RMSNorm(x + Mixer(x))", "x1 = x + Mixer(RMSNorm(x))", "x1 = Mixer(x) with the stream discarded", "x1 = RMSNorm(x) alone"]
    correctIndex: 1
