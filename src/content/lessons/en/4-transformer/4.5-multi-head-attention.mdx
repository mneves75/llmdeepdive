...
quiz:
  - question: "Qwen3.8-27B has 24 query heads but only 4 KV heads. How many query heads share each key/value head?"
    options: ["24, all of them", "6, since 24 ÷ 4 = 6", "4", "1, sharing never happens"]
    correctIndex: 1
  ...
  - question: "In a decoder like Qwen3.8-27B, what constraint applies inside every head's attention computation (lesson 4.4)?"
    options: ["Heads may read future tokens as long as others do not", "Only the first head is masked", "The causal mask — each head's positions may only attend to their prefix", "Masking is applied after the output projection"]
    correctIndex: 2
  ...
  - question: "Does using 24 heads guarantee 24 distinct linguistic functions?"
    options: ["Yes, by architectural definition", "Yes, if causal masking is used", "No, because heads have no learned weights", "No, heads may be redundant or encode distributed features"]
    correctIndex: 3
