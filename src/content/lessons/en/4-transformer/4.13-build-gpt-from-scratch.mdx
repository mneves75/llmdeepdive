...
quiz:
  - question: "Why is the triangular mask registered as a buffer?"
    options: ["It becomes an optimizer parameter", "It follows the module across devices but is not trainable", "It stores generated text", "It replaces position embeddings"]
    correctIndex: 1
  ...
  - question: "Using the last lesson's five axes, how is the minimal GPT classified?"
    options: ["Encoder–decoder with cross-attention, dense, hybrid", "Decoder-only mixture-of-experts, multimodal", "Decoder-only, next-token objective, dense, uniform attention mixer, text-only", "Encoder-only with a corruption objective"]
    correctIndex: 2
  ...
  - question: "At the 27B end of the parameter ladder, which component holds the most parameters?"
    options: ["The token embedding table alone", "The causal masks", "The optimizer's learning-rate schedule", "The gated FFNs, roughly 17B across the 64 layers"]
    correctIndex: 3
