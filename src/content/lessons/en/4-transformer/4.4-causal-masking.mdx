...
quiz:
  ...
  - question: "Why does adding negative infinity to a logit zero out its attention weight (lesson 4.3)?"
    options: ["Negative numbers are deleted from memory", "Softmax exponentiates logits and exp(−∞) is 0, so the position gets zero normalized weight", "The value vector is set to zero directly", "Because scores are divided by √d_k"]
    correctIndex: 1
  ...
  - question: "Why can causal language models still train all positions in parallel?"
    options: ["Each position secretly reads the answer", "The mask removes all dependencies", "The whole triangular score matrix is computed in one batched operation", "Training uses no sequence"]
    correctIndex: 2
