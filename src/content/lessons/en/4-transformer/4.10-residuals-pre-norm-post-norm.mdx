...
quiz:
  ...
  - question: "How does RMSNorm differ from classic LayerNorm?"
    options: ["It normalizes across the batch dimension", "It requires a bias term at every position", "It replaces the residual connection entirely", "It divides by sqrt(mean(x²) + epsilon) with a learned scale, without subtracting the feature mean"]
    correctIndex: 3
    explanation: "RMSNorm skips mean subtraction and rescales by sqrt(mean(x²) + epsilon), a cheaper operation that works well in large decoder stacks."
