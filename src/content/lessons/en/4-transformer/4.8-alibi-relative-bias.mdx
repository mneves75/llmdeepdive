...
quiz:
  ...
  - question: "How does RoPE (lesson 4.7) achieve relative-position dependence, in contrast with ALiBi's additive bias?"
    options: ["It adds a learned vector to token embeddings", "It rotates queries and keys so their dot product depends on the position difference", "It multiplies softmax outputs by distance", "It sorts keys by distance before attention"]
    correctIndex: 1
  ...
  - question: "Why use different ALiBi slopes across heads?"
    options: ["To make every head identical", "To remove causal order", "To provide several distance preferences, from strongly local to tolerant of long range", "To learn absolute token IDs"]
    correctIndex: 2
