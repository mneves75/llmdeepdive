...
quiz:
  ...
  - question: "Which family includes cross-attention between two stacks?"
    options: ["Encoder-only", "Encoder–decoder", "Decoder-only", "Hybrid decoder-only"]
    correctIndex: 1
  ...
  - question: "In the previous lesson's terms, what do a DeltaNet block and an attention block share that lets one model contain both?"
    options: ["The same score matrix over all past tokens", "A single set of weights reused in every layer", "The block interface: pre-norm reads from a shared residual stream and each mixer returns an update of the model width", "Nothing; they run in separate models"]
    correctIndex: 2
