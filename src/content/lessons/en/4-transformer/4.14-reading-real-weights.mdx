...
quiz:
  ...
  - question: "You encounter q_proj with shape 12288×5120 and conclude the conversion is broken, since 24 heads × 256 is 6144. What is the correct diagnosis?"
    options: ["Nothing is broken: these layers are gated, so q_proj emits 24 × 256 × 2 = 12288, each head owning 256 query values then 256 gate values", "The tensor is transposed and must be fixed to 5120×5120", "The file mixed up q_proj with the FFN", "Head dimension must equal hidden size divided by head count"]
    correctIndex: 0
