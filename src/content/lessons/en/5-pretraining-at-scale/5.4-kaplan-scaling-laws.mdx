...
quiz:
  ...
  - question: "Lesson 5.3 showed that 248,320 of Qwen3.8-27B's parameters-per-row go into the embedding and untied head. Why does that matter when reading a scaling curve plotted against N?"
    options: ["Embeddings must be counted twice on the N axis", "Kaplan-style fits use non-embedding parameters, so roughly 2.54 billion of this model's parameters are excluded from that axis", "The vocabulary changes the irreducible floor", "Untied heads make power laws inapplicable"]
    correctIndex: 1
  ...
  - question: "Why run many smaller scaling experiments?"
    options: ["To eliminate the need for held-out data", "To prove all architectures scale identically", "To estimate trends before committing the largest compute budget", "To avoid recording hyperparameters"]
    correctIndex: 2
...
Here $x$ is non-embedding parameter count, dataset size, or training compute. $L_\infty$ is the irreducible term — the entropy the data itself carries, which no model removes. $A x^{-\alpha}$ is the reducible part, and on log-log axes it becomes a straight line whose slope is $-\alpha$. A larger positive $\alpha$ means loss falls faster as that resource grows.
