...
quiz:
  - question: "Qwen3.8-27B is trained with multi-token prediction. What does that give you at inference time?"
    options: ["Four tokens emitted per step for free, with no verification", "A built-in draft model whose accepted guesses can accelerate speculative decoding", "A bidirectional encoder", "A larger vocabulary"]
    correctIndex: 1
  ...
  - question: "In the mini-GPT of lesson 4.13, why are the targets the input sequence shifted by one position?"
    options: ["Because the tokenizer reverses the string", "Because embeddings are untied", "Because the loss at position i must score the prediction of the token at position i+1 under a causal mask", "Because the optimizer requires it"]
    correctIndex: 2
  ...
  - question: "What distinguishes span corruption from independent token masking?"
    options: ["It exposes future target tokens", "It requires no tokenizer", "It changes model weights without loss", "It removes contiguous spans and predicts their contents"]
    correctIndex: 3
...
Run one step of the same arithmetic backwards from a plausible converged value. For a trained model at mean loss 2.0 nats, $e^{-2.0}\approx 0.135$ is the **geometric mean** of the probabilities assigned to the correct next tokens — roughly one in seven as an effective probability — while uniform prediction gives $1/248{,}320\approx 0.000004$. The ratio is about 33,000 in geometric-mean assigned probability. It is not the arithmetic mean of per-token probabilities.
