...
quiz:
  ...
  - question: "Lesson 5.7 left about 6.75 GB of persistent state per rank under ZeRO-3 on 64 ranks. Why might a team still add tensor parallelism on top?"
    options: ["Because ZeRO-3 cannot shard optimizer moments", "Sharding state does not shard the computation of one layer, so very large matrices and long sequences still exceed a single rank's compute and activation budget", "Because tensor parallelism removes the need for gradients", "Because FSDP requires a tensor-parallel group"]
    correctIndex: 1
  ...
  - question: "What is a pipeline bubble?"
    options: ["An attention probability", "A tokenizer fallback byte", "Stage idle time while the pipeline fills, drains, or waits", "An optimizer moment"]
    correctIndex: 2
