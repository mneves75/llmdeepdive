...
O primeiro portão é aritmética, não software. Os pesos custam 54 GB em bf16 (as lições 7.2 e 9.3 são donas dessa conta). Numa GPU de 80 GB isso deixa $80 - 54 = 26$ GB, cerca de 24 GiB, de pool de KV — a 64 KiB por token, $24\ \text{GiB} \div 64\ \text{KiB} \approx 390{,}000$ tokens de contexto de attention no pool de KV, suficiente para aproximadamente uma dúzia de sessões de 32K, como a lição 7.8 calculou. Se seu orçamento de hardware é uma placa de 24 ou 48 GB, esta lição inteira é prematura: você está no caminho dos artefatos quantizados que passa pela lição 7.12 e pelos runners locais da Track 8, e não escolhendo entre engines de data center.
