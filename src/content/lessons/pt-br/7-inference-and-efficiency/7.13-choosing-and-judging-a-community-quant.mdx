...
O formato dessa figura é a lição 7.3 tornada visível. O decode relê aproximadamente todo o conjunto de pesos para cada token que gera, então é limitado por largura de banda, e encolher os pesos ataca diretamente sua restrição: a geração quase triplica conforme os bits por peso caem de 16 para 2. O prefill é limitado por computação, e a quantization não multiplica unidades aritméticas, então ele permanece plano.

Leia a consequência com atenção, porque é o oposto da intuição com que as pessoas chegam. Dentro de uma mesma família de formatos e num mesmo backend, **menos bits geralmente decodificam mais rápido**, então um build de 6 bits não é a escolha rápida — é a escolha que abre mão de alguma velocidade para manter mais fidelidade, e o de 4 bits é o que abre mão de fidelidade para ir mais rápido. Quem descreve 6 bits como um bom equilíbrio está negociando exatamente nesse eixo, quaisquer que sejam as palavras usadas.

A generalização para por aí, e a mesma tabela publicada mostra por quê: a ordenação não é monotônica entre famílias. As consultas a codebook que permitem a um i-quant se manter coerente em dois bits também custam tempo de decode que um k-quant nunca paga, então um arquivo i-quant menor pode gerar mais devagar que um k-quant maior na mesma máquina — a ressalva que a lição 8.7 levanta pelo lado do llama.cpp. Tamanho prevê velocidade apenas dentro de uma família, apenas num backend cujos kernels consomem aquele formato nativamente, e apenas até você medir.
