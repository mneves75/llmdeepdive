...
quiz:
  - question: "Um artefato GGUF é rotulado Q4_K_M e mede 4,8944 bits por peso. O que explica a diferença entre 4 e 4,8944?"
    options: ["O arquivo armazena cada código de 4 bits preenchido até um byte completo", "Escalas de bloco, metadados de super-bloco e tensores que a receita mantém deliberadamente em precisão mais alta entram todos na média", "O tokenizer e o chat template são contados como pesos", "A medição inclui o KV cache no comprimento de contexto padrão"]
    correctIndex: 1
  ...
  - question: "Usando a lição 7.3, por que quantizar aumenta bastante a velocidade de geração de texto e não faz nada pelo processamento do prompt?"
    options: ["O prefill pula inteiramente os blocos feed-forward", "Kernels quantizados só são implementados para o caminho de decode em todos os runtimes", "O decode é limitado por largura de banda de memória e move os bytes de peso por token, enquanto o prefill é limitado por computação — e desempacotar pesos quantizados acrescenta aritmética ali, em vez de remover", "O processamento do prompt lê os pesos do disco, e não da memória"]
    correctIndex: 2
...
:::note
Bits por peso depende do modelo. Ele varia com o inventário de tensores, o tamanho do embedding, os tensores protegidos e os metadados. Os 4,8944 acima são uma medição publicada para este artefato Llama-3.1-8B; a lição 7.10 deliberadamente não infere um bpw exato do Qwen a partir do rótulo arredondado de 27B. Leia bpw como fato sobre um artefato, nunca como propriedade de um formato.
:::
