...
quiz:
  - question: "Um artefato GGUF é rotulado Q4_K_M e mede 4,8944 bits por peso. O que explica a diferença entre 4 e 4,8944?"
    ...
    correctIndex: 0
    explanation: "Bits por peso faz média do checkpoint inteiro, metadados inclusos, e é exatamente por isso que excede a largura nominal. As lições 7.9 e 8.7 aplicam o mesmo raciocínio ao Qwen3.8-27B e chegam a outra taxa — o efeito é geral, o número é por artefato."
  - question: "Usando a lição 7.3, por que quantizar aumenta bastante a velocidade de geração de texto e não faz nada pelo processamento do prompt?"
    options: ["O decode é limitado por largura de banda de memória e move os bytes de peso por token, enquanto o prefill é limitado por computação — e desempacotar pesos quantizados acrescenta aritmética ali, em vez de remover", "O prefill pula inteiramente os blocos feed-forward", "Kernels quantizados só são implementados para o caminho de decode em todos os runtimes", "O processamento do prompt lê os pesos do disco, e não da memória"]
    correctIndex: 0
    explanation: "A lição 7.3 separou as duas máquinas: o decode relê aproximadamente todo o conjunto de pesos por token gerado, então menos bytes armazenados atacam diretamente o gargalo dele. O prefill já satura as unidades aritméticas, e desquantizar é trabalho extra para elas — e é por isso que a tabela medida mostra o processamento do prompt não apenas plano, mas ligeiramente pior que o F16."
