...
quiz:
  - question: "Qual métrica visível ao usuário o prefill controla mais diretamente?"
    options: ["Inter-token latency", "Tamanho do vocabulário do tokenizer", "Tempo de download dos pesos", "Time to first token, já que ele abrange o processamento do prompt antes de existir qualquer saída"]
    correctIndex: 3
  ...
  - question: "Por que o tráfego de memória do decode em 48 das 64 camadas do Qwen3.8-27B permanece plano conforme o contexto cresce?"
    ...
    correctIndex: 0
    explanation: "O estado matricial de DeltaNet da referência em float32 ocupa 3 MiB por camada, 144 MiB no total, independentemente do comprimento da sequência; só as 16 camadas de attention leem um cache crescente."
citations:
...
O modelo mental durável: um processador paralelo de prompt entrega estado a um motor de tokens sensível à latência que sobretudo move memória. Tudo no resto desta trilha — attention ciente de IO, caches paginados, quantização, speculative decoding — é um ataque a um dos dois lados dessa fronteira.
