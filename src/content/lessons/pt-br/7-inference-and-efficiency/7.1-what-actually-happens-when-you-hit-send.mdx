...
quiz:
  - question: "Por que o prefill consegue processar todas as posições do prompt numa única passagem paralela?"
    options: ["O prompt não contém attention", "Self-attention causal permite que cada posição compute sua saída a partir das posições anteriores já conhecidas simultaneamente", "O modelo pula suas camadas durante o prefill", "Tokens do prompt não exigem projeções"]
    correctIndex: 1
  ...
  - question: "Sem speculative decoding nem MTP, uma resposta de 500 tokens exige cerca de quantas passagens de decode do target model?"
    options: ["Exatamente um", "Um por camada, ou seja, 64", "Cerca de 500 — um por token gerado", "Nenhum; o prefill emite a resposta inteira"]
    correctIndex: 2
    explanation: "O decoding autorregressivo de base finaliza um token por passagem do target. Métodos especulativos e multi-token podem amortizar uma passagem entre vários tokens aceitos."
citations:
