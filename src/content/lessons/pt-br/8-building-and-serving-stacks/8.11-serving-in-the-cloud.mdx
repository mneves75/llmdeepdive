...
teachBack:
  ...
  modelAnswer: "Tome 30.000 requisições por mês com 1.200 tokens de entrada e 400 de saída cada. A entrada são 36 milhões de tokens, a saída são 12 milhões. Em agosto de 2026 o preço do Cloudflare Workers AI para este modelo é USD 0,45 por milhão de tokens de entrada e USD 3,20 por milhão de saída, então a entrada custa USD 16,20, a saída custa USD 38,40 e o custo bruto de tabela é USD 54,60 antes da franquia diária gratuita. A saída é um quarto dos tokens e cerca de setenta por cento do custo, porque a geração é sequencial e limitada por memória enquanto o prefill é uma operação matricial paralela — a assimetria de preço espelha a assimetria da máquina. O thinking mode multiplica exatamente a metade cara: acrescentar 800 tokens de raciocínio por requisição leva a saída a 36 milhões e o custo bruto de tabela a cerca de USD 131, um aumento de 2,4 vezes vindo de uma única flag de configuração. Alugar é linear em tokens e capacidade dedicada é fixa, então o cruzamento é simplesmente o custo mensal do deployment dedicado dividido pelo custo gerenciado por requisição — abaixo desse volume você está pagando por silício ocioso, acima dele você está pagando margem em cada token."
quiz:
  - question: "Uma carga envia 40 milhões de tokens de entrada e 10 milhões de tokens de saída num mês. Usando os preços de agosto de 2026 do Workers AI para este modelo, qual é o custo bruto de tabela antes da franquia diária gratuita e qual metade domina?"
    options: ["USD 22,50 no total, já que uma taxa combinada se aplica aos 50 milhões de tokens","USD 32,00 de entrada mais USD 18,00 de saída, então USD 50,00 com a entrada dominando","USD 160,00, já que tokens de saída também são cobrados à taxa de entrada","USD 18,00 de entrada mais USD 32,00 de saída, então USD 50,00 com a saída dominando apesar de ser um quinto dos tokens"]
    correctIndex: 3
    explanation: "Antes da franquia diária gratuita, 40 milhões a USD 0,45 por milhão são USD 18,00 e 10 milhões a USD 3,20 por milhão são USD 32,00. A saída é 20 por cento do volume e 64 por cento desse custo bruto de tabela; a fatura real depende da distribuição diária do tráfego."
...
A lição 8.12 põe esta opção ao lado de todo stack local e auto-hospedado da trilha e dá a você um único framework para escolher entre eles.
