...
quiz:
  - question: "Onde um stack compiler-first mais plausivelmente supera um stack de kernels ajustados à mão, e onde ele mais plausivelmente perde?"
    options: ["Ganha em toda parte, porque compilação é estritamente melhor que código escrito à mão","Ganha só em batch sizes muito grandes e perde nos pequenos","Ganha onde overhead de lançamento e operações pequenas não fundidas dominam, como decode com batch baixo; perde onde um kernel de fornecedor já satura a banda de memória em batch alto","Ganha na qualidade do texto gerado e perde em throughput"]
    correctIndex: 2
  ...
  - question: "Lembrando a lição 8.4, quais responsabilidades do vLLM um serving stack ainda precisa prover mesmo que um compilador cuide da fusão de kernels?"
    options: ["Tokenização da string de entrada","Escolha da temperatura de sampling","Download dos pesos a partir do hub de modelos","Continuous batching e alocação de KV cache paginado entre requisições concorrentes"]
    correctIndex: 3
...
A ideia durável é uma questão de fronteira: quem é dono da otimização, o humano ou o compilador? Todo stack desta trilha responde isso em algum ponto dessa linha, e a lição 8.12 põe todos eles lado a lado de uma vez.
