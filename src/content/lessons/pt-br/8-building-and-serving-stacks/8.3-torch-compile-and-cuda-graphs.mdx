...
quiz:
  - question: "Pela lição 7.3, por que a fase de decode é o alvo natural do graph capture enquanto o prefill em geral não é?"
    options: ["O prefill não tem kernels a capturar","O decode repete um passo curto e uniforme para cada token, enquanto o prefill é compute-bound com shapes dependentes da entrada que forçariam recaptura constante","O decode é compute-bound e o prefill é memory-bound","O graph capture só funciona na primeira requisição"]
    correctIndex: 1
  ...
  - question: "O que o CUDA graph capture exige da memória que ele toca?"
    options: ["Que todos os tensors estejam na CPU","Que o modelo seja quantizado antes","Endereços estáticos — o graph reproduzido reexecuta os lançamentos gravados contra os mesmos buffers, então os tensors não podem ser realocados entre passos","Nada; os endereços são resolvidos no momento da reprodução"]
    correctIndex: 2
  ...
  - question: "Você embrulha o modelo em torch.compile e cronometra a chamada seguinte, que sai mais lenta que o eager. O que deu errado?"
    options: ["A compilação sempre deixa modelos mais lentos","Os pesos foram corrompidos pela compilação","O torch.compile não suporta bf16","A primeira chamada inclui o tracing e a geração de código; você mediu o compilador, não o modelo compilado"]
    correctIndex: 3
