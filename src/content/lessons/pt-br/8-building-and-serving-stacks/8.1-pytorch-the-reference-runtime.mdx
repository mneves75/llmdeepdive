...
quiz:
  - question: "O bloco clássico da lição 4.11 combina um token mixer com uma FFN position-wise. Percorrendo as 64 camadas de decoder do Qwen3.8-27B, o que permanece igual em todas elas?"
    options: ["Nada — cada camada tem um shape diferente","Os dois RMSNorms e a FFN com gating; só o token mixer alterna entre attention e Gated DeltaNet","O token mixer; a FFN é que alterna","Apenas a conexão residual"]
    correctIndex: 1
  ...
  - question: "Nas camadas de attention, qual é a largura de saída do peso q_proj?"
    options: ["6144, porque 24 query heads em head dimension 256 dão 24 × 256","5120, porque projeções são sempre quadradas","12288, porque as camadas têm porta: 24 × 256 para as queries mais o mesmo de novo para a porta","17408, o intermediate size da FFN"]
    correctIndex: 2
  ...
  - question: "Por que o PyTorch eager é um servidor de produção ruim para este modelo, mesmo sendo a referência?"
    options: ["Ele calcula uma matemática diferente da dos serving engines","Ele não consegue carregar pesos em bf16","Ele só roda em CPU","Ele despacha cada operação separadamente, aloca por requisição e não tem KV paginado nem continuous batching, então fica limitado por launches e desperdiça memória"]
    correctIndex: 3
...
Aqui está o retorno concreto de ler uma árvore de módulos em vez de uma ficha técnica. Percorra as 64 camadas, pergunte a cada uma que tipo de mixer ela abriga, e conte: 16 de attention, 48 de DeltaNet. Essa contagem não é curiosidade — é a entrada da derivação de cache da lição 7.2. Dezesseis camadas guardam keys e values por token; quarenta e oito guardam um estado cujo tamanho não depende de quão longa a conversa fica. Os 64 KiB por token de que todo plano de capacidade desta trilha e da próxima depende é algo que você pode *verificar* percorrendo esta árvore, em vez de aceitar por fé.
