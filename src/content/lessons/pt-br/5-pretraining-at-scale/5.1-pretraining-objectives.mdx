...
quiz:
  - question: "O Qwen3.8-27B é treinado com multi-token prediction. O que isso dá a você em tempo de inferência?"
    options: ["Quatro tokens emitidos por passo de graça, sem verificação", "Um draft model embutido cujos palpites aceitos podem acelerar o speculative decoding", "Um encoder bidirecional", "Um vocabulário maior"]
    correctIndex: 1
  ...
  - question: "No mini-GPT da lição 4.13, por que os alvos são a sequência de entrada deslocada em uma posição?"
    options: ["Porque o tokenizer inverte a string", "Porque os embeddings são untied", "Porque a loss na posição i precisa pontuar a predição do token na posição i+1 sob uma máscara causal", "Porque o optimizer exige isso"]
    correctIndex: 2
  ...
  - question: "O que distingue span corruption de mascaramento independente de tokens?"
    options: ["Ela expõe tokens-alvo futuros", "Ela não requer tokenizer", "Ela muda os pesos do modelo sem loss", "Ela remove trechos contíguos e prevê o conteúdo deles"]
    correctIndex: 3
...
Rode um passo da mesma aritmética de trás para frente a partir de um valor convergido plausível. Para um modelo treinado com loss média de 2,0 nats, $e^{-2.0}\approx 0.135$ é a **média geométrica** das probabilidades atribuídas aos próximos tokens corretos — aproximadamente um em sete como probabilidade efetiva —, enquanto a predição uniforme dá $1/248{,}320\approx 0.000004$. A razão é de cerca de 33.000 na média geométrica da probabilidade atribuída. Ela não é a média aritmética das probabilidades por token.
...
A pergunta duradoura é a que se carrega para o resto desta trilha: que jogo de predição consome cada unidade de compute? Para este modelo a resposta é next-token prediction sobre 248.320 classes, com heads de MTP olhando mais adiante e patches de visão compartilhando a sequência. A arquitetura restringe quais jogos são possíveis; o objetivo escolhe o que é recompensado em cada token de treino que houve — quantos quer que tenham sido, número que este modelo nunca publicou.
