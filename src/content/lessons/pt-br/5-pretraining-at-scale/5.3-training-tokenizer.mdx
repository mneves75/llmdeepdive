...
summary: "Treinar um tokenizer fixa um vocabulário que todo checkpoint posterior precisa compartilhar; o Qwen3.8-27B usa IDs até 248.076 em matrizes de 248.320 linhas, um alinhamento observado cuja motivação histórica não foi publicada."
tier: "advanced"
locale: "pt-br"
...
teachBack:
  prompt: "Explique como um tamanho-alvo de vocabulário é escolhido, diferencie o inventário documentado do tokenizer do Qwen3.8-27B da contagem de linhas das matrizes e explique o que a divisibilidade observada permite sem atribuir uma justificativa não publicada."
  modelAnswer: "O tamanho do vocabulário negocia fertilidade contra custo de matriz: um inventário maior pode encurtar sequências e melhorar cobertura multilíngue, mas cada linha custa parâmetros tanto no embedding quanto numa output head untied. No tokenizer fixado do Qwen, os tokens do modelo e os added tokens usam IDs até 248.076, enquanto o config.json aloca 248.320 linhas nas matrizes, deixando 243 linhas não usadas por esse tokenizer. O tamanho alocado é 512 × 485, portanto pode ser particionado igualmente por graus de tensor parallel em potência de dois até 512. Padding para alinhamento paralelo é uma explicação plausível, mas a Qwen não publicou por que escolheu esse tamanho. Depois do treino, o mapeamento token-para-ID continua fazendo parte do contrato do checkpoint."
quiz:
  - question: "O que o tokenizer e o config publicados do Qwen3.8-27B permitem afirmar sobre as matrizes de 248.320 linhas?"
    options: ["248.320 é o número exato de palavras distintas do corpus", "O model card diz que o padding foi escolhido para tensor parallelism", "O tokenizer não pode exceder 2^18 IDs", "O tokenizer usa IDs até 248.076, deixando 243 linhas de matriz sem uso; 248.320 é divisível por 512, mas a razão da escolha não foi publicada"]
    correctIndex: 3
    explanation: "Inventário, contagem alocada, cauda não usada e divisibilidade são fatos inspecionáveis. Tratar padding para tensor parallel como a razão histórica é inferência, não afirmação documentada da Qwen."
  ...
  - question: "O que precisa permanecer estável ao carregar um checkpoint?"
    options: ["Apenas o tamanho do vocabulário", "O mapeamento token-para-ID e a semântica dos tokens especiais", "Apenas a normalização para minúsculas", "A ordem dos batches de treino"]
    correctIndex: 1
...
:::insight
Um config lançado reporta a alocação da matriz, não necessariamente a quantidade de IDs emitidos por seu tokenizer. Neste artefato fixado, os números observados são 248.077 IDs usados e 248.320 linhas. Inspecione sempre os dois arquivos antes de interpretar a diferença.
:::
