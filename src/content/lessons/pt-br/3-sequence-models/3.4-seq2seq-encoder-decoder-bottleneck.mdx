...
teachBack:
  prompt: "Descreva o treino e a inferência do encoder-decoder recorrente clássico, depois explique as duas falhas distintas que a interface de vetor fixo causa, por que alargar o vetor alivia apenas parcialmente a capacidade e por que não repara o caminho do gradiente nem a interface de resumo único."
...
quiz:
  - question: "A lição 3.3 mostrou que o sinal de longo alcance de uma LSTM decai como um produto de forget gates. Num modelo seq2seq com fonte de 40 tokens e alvo de 45 tokens, qual é o comprimento desse produto para o primeiro token da fonte?"
    options: ["45 passos, apenas o decoder", "Cerca de 85 passos — o encoder inteiro mais o decoder inteiro, já que a única rota passa pelo estado final do encoder", "Um passo, porque o estado do encoder é passado diretamente", "Não depende dos comprimentos"]
    correctIndex: 1
  ...
  - question: "Por que o gargalo do vetor fixo não é resolvido tornando o vetor muito mais largo?"
    options: ["Vetores mais largos não podem ser treinados", "O decoder ignora completamente o estado do encoder", "A interface continua exigindo um único resumo, fixado antes de o decoding começar, para servir a todo passo do alvo, independentemente do que cada passo precise", "Cross-entropy exige comprimentos iguais de fonte e alvo"]
    correctIndex: 2
  ...
  - question: "O que condiciona o primeiro passo do decoder no seq2seq clássico?"
    options: ["Tokens futuros do alvo", "Um rótulo aleatório de teste", "Apenas o comprimento da fonte", "A representação final do encoder junto com um token de início"]
    correctIndex: 3
