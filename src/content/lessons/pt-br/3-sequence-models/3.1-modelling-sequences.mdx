...
quiz:
  ...
  - question: "Por que um estado de tamanho fixo impõe um teto rígido, e não apenas uma dificuldade?"
    options: ["Estados fixos não podem ser treinados com backpropagation", "Se uma tarefa exige levar adiante mais bits do que o estado consegue armazenar, nenhuma quantidade de treino faz caber", "Estados fixos não conseguem representar números negativos", "O teto só vale para sequências curtas"]
    correctIndex: 1
  ...
  - question: "O que caracteriza um modelo de sequência causal?"
    options: ["Ele não usa parâmetros", "Toda sequência tem o mesmo comprimento", "A representação na posição t não pode depender de nenhuma posição depois de t", "Ele prevê exatamente uma classe"]
    correctIndex: 2
