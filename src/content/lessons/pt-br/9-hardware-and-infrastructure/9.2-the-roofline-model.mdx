...
quiz:
  ...
  - question: "A lição 7.2 derivou 64 KiB de key-value cache por token para o Qwen3.8-27B. Conforme a conversa cresce, como as 16 camadas de full attention se movem no gráfico roofline?"
    options: ["Para a esquerda, porque os bytes de cache lidos por passo de decode crescem enquanto a aritmética por passo não", "Para a direita, porque contextos mais longos acrescentam aritmética mais rápido do que bytes", "Ficam fixas, porque o cache é comprimido", "Saem do gráfico, porque attention não tem roofline"]
    correctIndex: 0
    explanation: "Cada nova query atende sobre um histórico retido cada vez maior, então os bytes por passo sobem aproximadamente linear com o contexto enquanto as operações úteis por byte caem."
citations:
