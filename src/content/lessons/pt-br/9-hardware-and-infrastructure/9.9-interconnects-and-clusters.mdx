...
quiz:
  - question: "A lição 5.8 descreveu tensor parallelism como dividir matrizes de pesos entre dispositivos. Para o Qwen3.8-27B, qual dimensão limita o grau limpo de tensor parallel em 4?"
    options: ["O hidden size 5120","O tamanho intermediário 17408 da FFN","Os 4 KV heads da grouped-query attention","O vocabulário com padding de 248.320"]
    correctIndex: 2
  ...
  - question: "Por que um payload de all-reduce de cerca de 1,9 MiB por token ainda limita o throughput num interconnect lento?"
    options: ["Porque 1,9 MiB excede a bandwidth do PCIe","Porque o payload precisa ser enviado a todo nó do cluster","Porque o all-reduce exige transferir o KV cache inteiro","Porque o custo é dominado pela latência de 128 sincronizações em série por token, e não pelos bytes"]
    correctIndex: 3
