...
teachBack:
  ...
  modelAnswer: "O runtime da MI300X reporta cerca de 192 GiB de HBM. Os pesos em bf16 custam 50,3 GiB e uma reserva de runtime cerca de 8 GiB, deixando aproximadamente 133,7 GiB (136.909 MiB), o que dá em torno de 208 sequências concorrentes de 8.192 tokens a 656 MiB cada, ou cerca de oito sequências no contexto completo de 262.144 tokens a 16.528 MiB cada. Isso tira o sharding do projeto para cargas que uma peça de 80 GB não conseguiria hospedar sem ele. Capacidade sozinha não estabelece a latência por token; tráfego de bytes ativos, bandwidth e overhead de runtime estabelecem: a 5,3 TB/s informados pelo fabricante, a aproximação de tráfego do checkpoint completo de 54 GB por passo de decode dá um quociente pelo tamanho do checkpoint perto de 10,2 milissegundos, ou cerca de 98 tokens por segundo como heurística de planejamento, e tensor parallel sobre duas peças de 80 GB na verdade superaria isso ao dividir pela metade os bytes de peso que cada dispositivo lê, ao custo de um all-reduce a cada camada. A ressalva de software é que o suporte de ROCm precisa ser verificado por versão de stack em vez de presumido: cobertura de kernels, suporte a formatos de quantização e kernels fundidos para as camadas de Gated DeltaNet de um modelo lançado em agosto de 2026 podem estar atrás do caminho CUDA."
quiz:
  - question: "Um acelerador de 192 GB comporta o Qwen3.8-27B mais cerca de oito sequências de contexto completo sem sharding. O que isso compra?"
    options: ["Time to first token menor","Um teto aritmético maior para o prefill","Concorrência e deployment mais simples; a latência por token ainda depende do tráfego de bytes ativos, da bandwidth e do overhead de runtime","Tanto mais concorrência quanto decode de fluxo único proporcionalmente mais rápido"]
    correctIndex: 2
    explanation: "Capacidade e bandwidth são os dois eixos independentes da lição 9.4; um pool maior admite mais usuários enquanto a latência por token ainda depende dos bytes ativos por segundo e do overhead de runtime."
  - question: "Um time quer o chat de usuário único mais rápido possível e considera uma peça de 192 GB a 5,3 TB/s informados pelo fabricante contra duas peças de 80 GB a 3,35 TB/s cada em tensor parallel. Qual raciocínio está correto?"
    options: ["A peça única é sempre mais rápida, porque sharding acrescenta latência","São idênticos, porque o que importa é a bandwidth total","Nenhum consegue passar de 60 tokens por segundo","O par pode ser mais rápido, porque cada dispositivo lê apenas cerca de 27 GB por passo, aproximadamente 8 milissegundos, embora um all-reduce por camada consuma parte do ganho"]
    correctIndex: 3
...
$$
192\ \text{GiB} - 50.3\ \text{GiB} - 8\ \text{GiB} \ =\ 133.7\ \text{GiB} \approx\ 136{,}909\ \text{MiB}.
$$
...
$$
\frac{136{,}909\ \text{MiB}}{16{,}528\ \text{MiB}} \approx 8\ \text{sequências de contexto completo},
$$
...
$$
t_{\text{estimativa do checkpoint}} \approx\ \frac{54\times10^{9}\ \text{B}}{5.3\times10^{12}\ \text{B/s}} \approx\ 10.2\ \text{ms} \quad\Longrightarrow\quad \approx 98\ \text{tokens por segundo},
$$
...
:::caveat
Todo número de hardware aqui é informado pelo fabricante em agosto de 2026 e descreve uma família, não uma peça. Capacidade, bandwidth e contagem de compute units diferem entre SKUs MI3xx e entre as plataformas que os hospedam, e tipos de instância em nuvem podem expor dispositivos particionados. Rederive o orçamento com os números reportados pelo runtime na instância que você alugar, e trate os quocientes de tokens por segundo desta lição como heurísticas de ordem de grandeza, não limites rígidos.
:::
