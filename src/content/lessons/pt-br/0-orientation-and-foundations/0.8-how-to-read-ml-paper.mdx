...
quiz:
  - question: "O model card do Qwen3.8-27B reporta 89,2 no GPQA Diamond. Qual redação é defensável?"
    options: ["89,2 no GPQA Diamond significa que ele acerta 89,2 por cento das perguntas de ciência na prática","O Qwen3.8-27B alcança 89,2 no GPQA Diamond, provando capacidade científica de nível de pós-graduação","O fornecedor reporta 89,2 no GPQA Diamond; o harness e as configurações de sampling não são especificados e não há reprodução independente disponível","O Qwen3.8-27B é o modelo aberto mais forte em raciocínio científico"]
    correctIndex: 2
  ...
  - question: "Não existe relatório técnico para o Qwen3.8-27B. O que se segue disso quanto aos seus dados de treinamento?"
    options: ["Podem ser recuperados lendo o arquivo de configuração","Presume-se que sejam os mesmos da geração anterior","Podem ser inferidos a partir dos scores de benchmark","Nada pode ser afirmado honestamente sobre eles, porque nenhuma informação desse tipo foi publicada"]
    correctIndex: 3
  ...
  - question: "A lição 0.7 carregou a configuração do modelo para ler hidden_size, num_hidden_layers e vocab_size. Onde esses números ficam na hierarquia de evidência?"
    options: ["São primários e diretamente verificáveis para uma revisão fixada do artefato, que deve acompanhar a afirmação","Não são confiáveis sem um relatório técnico","São afirmações do fornecedor e precisam das mesmas ressalvas que scores de benchmark","São secundários, já que a configuração é gerada a partir dos pesos"]
    correctIndex: 0
    explanation: "A configuração acompanha os pesos e descreve os tensores que precisam existir para o modelo carregar, então ela pode ser conferida contra o índice de pesos. Isso é evidência mais forte do que um score autorreportado, mas continua sendo um fato versionado sobre um artefato específico."
citations:
