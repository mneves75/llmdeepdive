...
summary: "Um modelo de linguagem mapeia contexto para scores e probabilidades de próximo token; o Qwen3.8-27B tem 248.320 linhas de saída, enquanto seu tokenizer publicado mapeia 248.077 ids."
tier: "foundations"
locale: "pt-br"
prerequisites: []
...
teachBack:
  ...
  modelAnswer: "Um forward pass recebe o contexto até aqui e devolve 248.320 scores brutos, um por linha do tensor de saída. O tokenizer publicado mapeia 248.077 ids; os artefatos não especificam como todo runtime trata os outros 243 logits. Depois do mascaramento do runtime, o softmax transforma os scores selecionáveis numa distribuição de probabilidade que soma um. O modelo não emite uma palavra: escolher uma a partir da distribuição é um passo separado de decodificação. Nada é recuperado por chave. A distribuição é computada a partir de pesos ajustados a texto, então uma continuação pode ser produzida para formulações que nunca apareceram em lugar algum, e em geral não há um documento-fonte único que se possa nomear para uma resposta."
quiz:
  - question: "Quantos números um forward pass do Qwen3.8-27B produz para uma única posição, antes da decodificação?"
    options: ["Um, o token escolhido","248.320, um por linha do tensor de saída","27 bilhões, um por parâmetro","5120, um por dimensão oculta"]
    correctIndex: 1
    explanation: "O tensor de saída do modelo tem 248.320 linhas, então ele produz esse número de scores por posição. Seu tokenizer publicado mapeia 248.077 ids; um runtime precisa decidir como tratar os 243 logits restantes antes de escolher um token mapeado."
  - question: "Por que prever repetidamente o próximo token consegue produzir um parágrafo coerente?"
    options: ["Tokens já contêm parágrafos inteiros","O modelo armazena todos os parágrafos possíveis","Cada token escolhido é anexado, então a previsão seguinte condiciona em um contexto mais longo","O tokenizador confere a exatidão factual"]
    correctIndex: 2
  ...
  - question: "Um modelo atribui probabilidade 0,93 a uma continuação factualmente falsa. O que esse número significa?"
    options: ["A afirmação foi verificada durante o treinamento","A afirmação é 93 por cento verdadeira","O modelo a recuperou de uma fonte confiável","O modelo considera aquela continuação um texto altamente provável, o que não é uma afirmação sobre o mundo"]
    correctIndex: 3
...
:::caveat
Trate qualquer probabilidade específica citada nesta lição como ilustrativa. A largura de saída de 248.320 é um fato publicado sobre o Qwen3.8-27B; a massa exata que ele coloca em qualquer token específico não é, e neste curso nunca inventamos medições.
:::
