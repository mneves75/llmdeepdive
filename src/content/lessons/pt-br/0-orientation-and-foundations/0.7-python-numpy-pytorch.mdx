...
quiz:
  - question: "Ler a configuração do Qwen3.8-27B dá vocab_size 248320. O que esse número descreve exatamente?"
    options: ["O número de tokens que o modelo consegue processar de uma vez","O número de parâmetros da camada de saída","O número de palavras distintas que o modelo conhece","O número de linhas da matriz de embedding, que pode estar preenchida acima do vocabulário efetivo do tokenizador"]
    correctIndex: 3
  ...
  - question: "Por que optimizer.zero_grad normalmente é chamado antes de backward?"
    options: ["O PyTorch acumula gradientes nos campos grad em vez de substituí-los","Ele move os tensores para a GPU","Ele seleciona a função de loss","Ele apaga os parâmetros do modelo"]
    correctIndex: 0
  ...
  - question: "A lição 0.6 mostrou que um learning rate de 1,1 sobre L(w) = w ao quadrado faz o parâmetro divergir. Se você visse esse comportamento num loop de treinamento real, o que o autograd teria lhe dito?"
    options: ["Ele reduziria o learning rate automaticamente","Nada; o autograd computa gradientes corretos para uma execução divergente exatamente como faz para uma convergente","Ele reportaria o gradiente como not-a-number imediatamente","Ele levantaria uma exceção no segundo passo"]
    correctIndex: 1
...
:::caveat
Ler um número não é o mesmo que entender o que ele conta. O valor 248.320 é o número de linhas de embedding. O tokenizer publicado mapeia 248.077 ids, deixando 243 linhas de tensor sem mapeamento. A divisibilidade por 512 é conveniente para hardware, mas os artefatos não documentam por que esse shape exato foi escolhido, que sinal essas linhas não mapeadas receberam nem como todo runtime as trata. Tome a configuração como autoridade sobre shapes de tensores e o tokenizer como autoridade sobre o mapeamento de texto para ids; a lição 1.2 mostra por que confundi-los produz uma afirmação plausível e errada.
:::
