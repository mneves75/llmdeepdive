...
quiz:
  - question: "A lição 2.7 mencionou que parâmetros de escala de normalização geralmente vão para um grupo sem weight decay no otimizador. Por que isso faz sentido?"
    options: ["Eles não têm gradientes", "Eles são inteiros", "Eles definem a escala de operação do sinal normalizado em vez de atuarem como pesos de features, então encolhê-los rumo a zero suprime sistematicamente a saída da camada", "Weight decay é indefinido para eles"]
    correctIndex: 2
  ...
  - question: "Num bloco residual pre-norm, o que a normalização de fato normaliza?"
    options: ["O residual stream, que portanto é mantido em escala unitária por todo o modelo", "Apenas os gradientes", "Os logits de saída do modelo", "A cópia do hidden state entregue à subcamada — o residual stream em si segue sem normalização"]
    correctIndex: 3
...
Esses mecanismos são infraestrutura, e importa que você os leia como tal. Nenhum deles decide o que o modelo aprende. Eles tornam a composição profunda numericamente sobrevivível para que os blocos de attention e feed-forward da trilha 4 possam gastar o orçamento de otimização aprendendo transformações úteis em vez de lutar contra o colapso. A lição 4.10 volta a esse arranjo com o bloco transformer completo à vista.
