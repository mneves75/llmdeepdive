...
com um ganho aprendido por feature $g$ e, neste modelo, $\varepsilon=10^{-6}$. Passe um vetor por ele. Digamos que o hidden state de um token na camada 40 tenha valor RMS de 12,0 ao longo das suas 5120 entradas. O denominador é $\sqrt{144+10^{-6}}=12.0$ em qualquer precisão que importe, então toda entrada é dividida por 12 e a subcamada vê um vetor de RMS unitário. Agora considere uma posição quase morta cujas entradas estão todas em torno de $10^{-5}$: a média dos quadrados é $10^{-10}$, mil vezes menor que $\varepsilon$, então o epsilon domina a raiz quadrada e a saída é escalada para baixo rumo a zero em vez de explodir por uma divisão por quase nada. É precisamente esse o trabalho do epsilon — ele não é um remendo de arredondamento, é a guarda que mantém a operação definida no fundo da faixa.
