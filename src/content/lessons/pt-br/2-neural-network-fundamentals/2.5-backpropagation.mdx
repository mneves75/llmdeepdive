...
summary: "Diferenciação em modo reverso obtém o gradiente de todo parâmetro numa única varredura cujo custo é da mesma ordem de avaliar o grafo — a única razão pela qual um modelo de 27 bilhões de parâmetros pode ser treinado."
tier: "foundations"
locale: "pt-br"
...
quiz:
  - question: "Para softmax com cross-entropy, da lição 2.4, qual é o gradiente da loss em relação aos logits?"
    options: ["O log negativo da probabilidade do alvo", "A distribuição prevista menos o alvo one-hot, p − y", "Sempre zero para a classe correta", "O softmax do gradiente"]
    correctIndex: 1
  ...
  - question: "Por que contribuições de gradiente são somadas num ponto de ramificação?"
    options: ["A soma substitui a regra da cadeia", "Só um caminho tem derivada", "O valor afeta a loss por múltiplos caminhos de jusante, e a regra da cadeia multivariada soma sobre todos eles", "O otimizador exige inteiros"]
    correctIndex: 2
  ...
  - question: "O que a retropropagação em si muda?"
    options: ["Ela tokeniza o batch", "Ela seleciona os rótulos", "Ela garante loss menor no teste", "Nada — ela computa gradientes, e um otimizador separado muda parâmetros"]
    correctIndex: 3
...
:::insight
Agora escale a figura para o modelo deste curso. O Qwen3.8-27B empilha 64 camadas, cada uma carregando um hidden state de 5120 de largura por token (Qwen3.8-27B Model Card, 2026). Um backward pass é uma única varredura desde a loss no topo descendo por todas as 64 camadas, e em cada uma delas ele produz tanto os gradientes das matrizes daquela camada quanto a mensagem repassada para baixo. O fato notável não é que isso seja rápido. É que o algoritmo percorre uma vez o grafo registrado em vez de rodar o modelo novamente para cada parâmetro — uma varredura, todos os gradientes, que é a única razão pela qual modelos desse tamanho existem.
:::
