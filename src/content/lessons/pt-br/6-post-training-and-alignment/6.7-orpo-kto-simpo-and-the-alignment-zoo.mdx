...
quiz:
  - question: "Qual método é projetado especificamente para usar feedback não pareado de desejável ou indesejável?"
    options: ["Apenas PPO", "Beam search", "FlashAttention", "KTO"]
    correctIndex: 3
  ...
  - question: "A lição 6.6 mostrou que a referência congelada da DPO ancora o objetivo. O que muda quando o SimPO a remove?"
    options: ["A âncora explícita da referência desaparece; normalização por comprimento, escala da recompensa e margem-alvo moldam a loss, mas não garantem proximidade com a policy original", "O método não consegue mais usar pares de preferência", "Log-probabilidades de resposta não podem mais ser calculadas", "A verossimilhança da resposta escolhida passa a subir garantidamente"]
    correctIndex: 0
    explanation: "O termo de referência fazia trabalho explícito de regularização, não de contabilidade. O SimPO tem uma escala de recompensa e uma margem-alvo, mas nenhuma delas limita rigidamente a divergência em relação à policy inicial."
  - question: "Qual é uma característica definidora do SimPO?"
    options: ["Um reward model separado obrigatório", "Um escore de policy sem referência, normalizado por comprimento, com uma margem-alvo", "Nenhum sinal de preferência", "Atualizar apenas o tokenizer"]
    correctIndex: 1
...
A loss pareada treina para que a recompensa escolhida supere a rejeitada por uma margem-alvo $\gamma$. A escala $\beta$ controla a força das diferenças de recompensa; nenhum dos dois parâmetros é uma restrição de policy de referência. Percorra o que a normalização por comprimento faz com $\beta=1$, porque não é um ajuste cosmético. Tome duas respostas ao mesmo prompt. A resposta A tem 40 tokens com log-probabilidade somada de $-60$. A resposta B tem 200 tokens com log-probabilidade somada de $-240$. Sob a pontuação somada, A pontua $-60$ e B pontua $-240$, então A vence com folga. Sob a normalização do SimPO, A tem média $-1.5$ por token e B tem média $-1.2$, então B vence. As mesmas duas respostas, o mesmo modelo, ordenação oposta — produzida inteiramente pela escolha da redução. Nenhuma das duas é "correta"; elas codificam crenças diferentes sobre se uma resposta longa deve ser penalizada pelo comprimento. É por isso que a interação com a política de truncamento e com a verbosidade desejada precisa ser avaliada nos seus dados, e não presumida de um artigo.
