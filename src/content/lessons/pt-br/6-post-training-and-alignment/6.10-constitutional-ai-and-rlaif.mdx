...
quiz:
  - question: "O que a constituição fornece?"
    options: ["Uma prova matemática de que o modelo é inofensivo", "Um tokenizer substituto", "Princípios explícitos usados para criticar, revisar ou ordenar comportamento", "Contexto ilimitado"]
    correctIndex: 2
  ...
  - question: "A lição 6.4 alertou que um reward model herda os confundidores do pipeline que produziu seus rótulos. Qual é o risco correspondente quando os rótulos vêm de um modelo jurado?"
    options: ["Rótulos de IA são mais ruidosos que rótulos humanos em toda dimensão", "Modelos jurados não conseguem produzir comparações pareadas", "O reward model deixa de precisar de rubrica", "Os pontos cegos do próprio jurado são herdados de forma sistemática em vez de aleatória, então os erros se correlacionam em vez de se cancelarem"]
    correctIndex: 3
