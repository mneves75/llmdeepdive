...
quiz:
  - question: "Que informação os logits do teacher fornecem além de um único rótulo duro?"
    options: ["Uma garantia de verdade factual", "O estado do otimizador do student", "Uma GPU física maior", "A probabilidade relativa atribuída a tokens alternativos"]
    correctIndex: 3
  ...
  - question: "A lição 7.10 fixa o arquivo Q4_K_M do modelo de texto do Qwen3.8-27B em 17,1 GB. Por que isso não torna quantization um substituto da distillation com um orçamento menor?"
    options: ["A quantization re-representa os mesmos 27B de parâmetros; se nenhum formato suportado de bits mais baixos atende às metas de memória, qualidade e latência, um student menor muda a contagem de parâmetros", "Modelos quantizados não podem ser servidos em hardware móvel de jeito nenhum", "A quantização remove o chat template do modelo", "Um modelo de 4 bits não tem logits para aprender"]
    correctIndex: 0
    explanation: "As técnicas agem em eixos diferentes. A quantization muda a representação dos pesos e pode descer abaixo de quatro bits; a distillation muda o que uma contagem menor de parâmetros aprende. Escolha distillation quando os quants suportados falharem na meta completa de memória, qualidade ou latência."
  - question: "O que é distillation no nível da sequência?"
    options: ["Copiar o hardware do teacher", "Treinar um student sobre saídas completas geradas ou selecionadas a partir de um teacher", "Remover todos os tokens de resposta", "Usar apenas rótulos aleatórios"]
    correctIndex: 1
