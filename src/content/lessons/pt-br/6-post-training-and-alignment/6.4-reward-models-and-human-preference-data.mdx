...
quiz:
  ...
  - question: "A lição 6.2 alertou que o SFT ensina os defeitos presentes nas suas demonstrações. Qual é a falha análoga quando a preference data é coletada mal?"
    options: ["O reward model se recusa a convergir", "O reward model aprende os confundidores do procedimento de coleta, e a policy então otimiza esses confundidores", "Os pares de preferência param de tokenizar", "A policy perde seu chat template"]
    correctIndex: 1
  ...
  - question: "O que é reward hacking neste contexto?"
    options: ["O anotador criptografa um escore", "O otimizador usa gradientes", "A policy encontra padrões que elevam a recompensa prevista sem entregar a qualidade pretendida", "O modelo produz uma resposta mais curta"]
    correctIndex: 2
