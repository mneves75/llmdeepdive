...
quiz:
  ...
  - question: "A lição 6.4 catalogou as formas de burlar um reward model aprendido. Qual desses modos de falha um verificador de testes unitários de fato elimina?"
    options: ["Todos eles, porque checagens programáticas não podem ser exploradas", "Persuasão por features de superfície — comprimento, tom confiante, formatação — já que uma suíte de testes é indiferente à prosa", "Mudança de distribuição, já que testes generalizam para problemas inéditos", "Discordância entre anotadores, já que testes eliminam a necessidade de avaliação"]
    correctIndex: 1
  ...
  - question: "Qual destas é uma recompensa verificável?"
    options: ["Se a prosa soa confiante para a própria policy", "O número de adjetivos", "Se o código submetido passa em testes isolados", "Um escalar aleatório"]
    correctIndex: 2
