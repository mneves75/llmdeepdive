...
order: 5
title: "RLHF com PPO"
summary: "O RLHF baseado em PPO otimiza uma policy de modelo de linguagem contra recompensa aprendida, aplicando clipping ao objetivo substituto e ancorando a policy a uma referência — maquinaria de controle que estabiliza a busca sem validar o que está sendo buscado."
tier: "advanced"
locale: "pt-br"
prerequisites: ["6.4-reward-models-and-human-preference-data"]
unlocks: []
analogy: "Um preparador de oratória premia o aluno por respostas mais claras, mas guarda uma gravação da linha de base competente do aluno. Além de uma faixa protegida, a planilha deixa de dar crédito extra por empurrar ainda mais um fraseado já melhorado; ela não impede fisicamente o aluno de mudá-lo. O PPO molda esse incentivo, o reward model é a nota do preparador, e a policy de referência é a gravação de base usada para penalizar deriva."
teachBack:
...
quiz:
  - question: "Por que incluir uma penalidade de KL contra a policy de referência?"
    options: ["Para tornar as recompensas perfeitamente objetivas", "Para desencorajar a policy otimizada de se afastar demais de uma policy de linguagem conhecida", "Para remover as probabilidades de token", "Para treinar o tokenizer"]
    correctIndex: 1
  ...
  - question: "A lição 6.4 mostrou que a loss do reward model depende apenas de diferenças de escore. Por que isso faz da recompensa de treino crescente um sinal fraco de sucesso?"
    options: ["Recompensa subindo significa que a penalidade de KL foi desativada", "O reward model não consegue pontuar respostas inéditas", "A escala é arbitrária e sem âncora, então um número subindo reporta movimento ao longo de um proxy, não ganho mensurável de qualidade", "Recompensas mais altas sempre indicam respostas mais curtas"]
    correctIndex: 2
  ...
  - question: "O que o objetivo substituto com clipping do PPO realmente faz?"
    options: ["Prende toda razão de probabilidade dentro da faixa", "Limita o número de palavras de cada resposta", "Valida o reward model", "Remove ganho adicional do objetivo quando uma razão se move favoravelmente além da faixa, sem limitar a razão de forma rígida"]
    correctIndex: 3
    explanation: "O objetivo com clipping achata o incentivo em direções específicas. Outras amostras e passos do otimizador ainda podem mover a razão para fora do intervalo, portanto não se trata de uma restrição rígida de trust region."
citations:
...
Passe o clip pelos números, porque sua assimetria surpreende as pessoas. Tome uma faixa de clip de $\epsilon=0.2$, de modo que a cópia cortada da razão fique entre 0,8 e 1,2. Suponha que um token tivesse probabilidade 0,20 sob a policy antiga e 0,30 sob a nova, dando $\rho_t = 1.5$, e que sua vantagem fosse $+2.0$. O termo sem clip é 3,0, o termo com clip é 1,2 vezes 2,0, ou 2,4, e o mínimo dos dois é 2,4. O objetivo substituto não ganha mais nada ao empurrar a probabilidade deste token para cima, então o incentivo desta amostra a continuar subindo desaparece. A razão ainda é 1,5: o PPO não a prendeu em 1,2. Agora mantenha a mesma razão e faça a vantagem $-2.0$. O termo sem clip é $-3.0$, o termo com clip é $-2.4$, e o mínimo é $-3.0$. O clip não achata o objetivo nessa direção — uma ação ruim cuja probabilidade já subiu continua sendo empurrada para baixo com força. Outras amostras compartilham os mesmos parâmetros, então seus gradientes podem mover essa razão ainda mais para fora da faixa nominal. O PPO aplica clipping a uma contribuição do objetivo, não ao passo do otimizador nem à policy resultante.
