...
quiz:
  - question: "Por que um base model capaz pode continuar uma instrução em vez de obedecê-la?"
    options: ["Seu tokenizer não consegue representar comandos", "Seu objetivo aprendeu continuações prováveis de texto, não um papel privilegiado de assistente", "Self-attention só funciona em documentos", "Ele não tem conhecimento de língua aprendido"]
    correctIndex: 1
  ...
  - question: "Numa camada de self-attention, quais projeções o post-training altera para instalar comportamento de assistente?"
    options: ["Novos heads de attention específicos de papel são criados para system, user e assistant", "A softmax é substituída por um roteador baseado em regras", "As mesmas projeções de query, key, value e saída que já existiam — seus valores se movem, nenhum mecanismo novo aparece", "Attention é desativada e substituída por uma tabela de respostas aprovadas"]
    correctIndex: 2
  ...
  - question: "O que o post-training muda primordialmente?"
    options: ["A definição de multiplicação de matrizes", "O tamanho físico de cada token", "A internet disponível durante o pretraining", "A distribuição de comportamentos que o modelo seleciona para os prompts"]
    correctIndex: 3
