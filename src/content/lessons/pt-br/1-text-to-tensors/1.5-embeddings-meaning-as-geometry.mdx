...
quiz:
  - question: "O Qwen3.8-27B tem 248.320 linhas de tensor e hidden size 5120, e não compartilha seus embeddings. Quantos parâmetros ficam no embedding de entrada e na projeção de saída somados?"
    options: ["Cerca de 1,271 bilhão","Cerca de 2,54 bilhões","Cerca de 27 bilhões","Cerca de 5120"]
    correctIndex: 1
  ...
  - question: "Por que dois tokens podem acabar próximos no espaço de embeddings?"
    options: ["Eles têm o mesmo número de letras","Seus ids são numericamente adjacentes","O treino descobriu que seus usos preditivos são relacionados","Eles significam sempre exatamente a mesma coisa"]
    correctIndex: 2
  ...
  - question: "A lição 1.4 mostrou que texto em português pode exigir mais tokens que seu equivalente em inglês. O que isso implica para as buscas de embedding desse texto?"
    options: ["Os embeddings são combinados num único vetor antes","As linhas buscadas têm qualidade pior","A tabela de embeddings precisa ser maior","Mais linhas são buscadas e mais posições entram na rede, então o mesmo conteúdo custa mais trabalho em cada camada"]
    correctIndex: 3
...
citations:
  ...
  - title: "Information Leakage in Embedding Models"
    authors: "Congzheng Song e Ananth Raghunathan"
    year: 2020
...
:::insight
A matriz de saída é o mesmo objeto que a lição 0.1 descreveu pelo outro lado. Produzir 248.320 logits brutos significa pegar o estado final de 5120 dimensões e computar um produto escalar contra cada linha de saída; o tokenizer mapeia 248.077 ids, e o tratamento dos outros 243 logits pelo runtime não é especificado por esses artefatos. Não compartilhar permite ao modelo aprender uma geometria para ler um token e outra para prevê-lo, que são trabalhos genuinamente diferentes: o que torna dois tokens intercambiáveis como *entrada* não é o que os torna concorrentes como *saída*.
:::
