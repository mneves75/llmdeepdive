...
quiz:
  - question: "No benchmark padrão de analogia, calcular king menos man mais woman e buscar a palavra mais próxima exclui as três palavras da consulta do conjunto de candidatos. Por que isso importa?"
    options: ["Uma reavaliação publicada mostra que manter as palavras da consulta entre os candidatos pode derrubar bastante a acurácia e levar várias categorias a zero","Garante que a resposta esteja gramaticalmente correta","Impede que os vetores sejam normalizados","Deixa a computação mais rápida"]
    correctIndex: 0
    explanation: "A exclusão de candidatos muda materialmente o resultado medido. Linzen reportou quedas drásticas sem ela, incluindo acurácia zero em várias categorias; reportar a regra faz parte do resultado."
  - question: "O skip-gram treina uma representação de palavra para ajudar a prever o quê?"
    options: ["O título do documento","Palavras de contexto próximas","Uma definição de dicionário","A grafia da palavra"]
    correctIndex: 1
  ...
  - question: "A lição 1.5 observou que o Qwen3.8-27B não compartilha seus embeddings de entrada e de saída. Que ideia mais antiga isso ecoa?"
    options: ["O GloVe usava duas matrizes de coocorrência","Modelos de n-gramas guardavam duas tabelas de contagem","O word2vec também mantinha tabelas de embedding de entrada e de saída separadas, porque ler uma palavra e prevê-la são trabalhos diferentes","Nada; embeddings não compartilhados são uma invenção recente sem precedente"]
    correctIndex: 2
...
:::note
O Qwen3.8-27B não tem estágio word2vec em lugar nenhum. Seu tensor de embedding com 248.320 linhas é otimizado de ponta a ponta com o modelo; o tokenizer mapeia 248.077 ids, e os artefatos publicados não revelam que sinal as 243 linhas não mapeadas receberam, sem nenhum pré-treino separado de vetores de palavra. O que sobrevive daquela era não é o algoritmo, e sim o achado: evidência distribucional, dado um espaço contínuo e um objetivo, se arruma sozinha em estrutura utilizável. E um detalhe estrutural sobrevive literalmente — a separação do word2vec entre tabela de entrada e tabela de saída é a mesma assimetria do embedding não compartilhado e do language-model head deste modelo, vista na lição 1.5, numa escala muito maior e aprendida de ponta a ponta em vez de isoladamente.
:::
