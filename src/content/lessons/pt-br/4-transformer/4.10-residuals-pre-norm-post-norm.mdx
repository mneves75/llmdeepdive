...
quiz:
  ...
  - question: "Como o RMSNorm difere do LayerNorm clássico?"
    options: ["Normaliza ao longo da dimensão de batch", "Exige um termo de bias em cada posição", "Substitui inteiramente a conexão residual", "Divide por sqrt(mean(x²) + epsilon) com uma escala aprendida, sem subtrair a média das features"]
    correctIndex: 3
    explanation: "O RMSNorm pula a subtração da média e reescala por sqrt(mean(x²) + epsilon), uma operação mais barata que funciona bem em pilhas decoder grandes."
