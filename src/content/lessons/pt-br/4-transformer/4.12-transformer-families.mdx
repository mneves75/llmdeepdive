...
quiz:
  ...
  - question: "Qual família inclui cross-attention entre duas pilhas?"
    options: ["Encoder-only", "Encoder–decoder", "Decoder-only", "Decoder-only híbrido"]
    correctIndex: 1
  ...
  - question: "Nos termos da lição anterior, o que um bloco DeltaNet e um bloco de attention compartilham que permite a um mesmo modelo conter os dois?"
    options: ["A mesma matriz de scores sobre todos os tokens passados", "Um único conjunto de pesos reutilizado em cada camada", "A interface do bloco: pre-norm lê de um residual stream compartilhado e cada mixer devolve uma atualização com a largura do modelo", "Nada; eles rodam em modelos separados"]
    correctIndex: 2
...
As famílias também diferem na economia de serving. Um encoder processa sua fonte uma vez; um encoder–decoder faz cache das saídas do encoder mas paga self- mais cross-attention enquanto decodifica; um decoder puro se apoia num KV cache que cresce com o contexto; um MoE movimenta pesos entre experts; um híbrido encolhe o cache crescente para sua minoria de attention. A trilha 7 quantifica essas diferenças — por ora basta que o rótulo da família preveja de onde vem a conta do serving.
