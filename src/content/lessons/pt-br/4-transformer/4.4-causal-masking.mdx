...
quiz:
  ...
  - question: "Por que somar menos infinito a um logit zera o peso de attention dele (lição 4.3)?"
    options: ["Números negativos são apagados da memória", "O softmax exponencia os logits e exp(−∞) é 0, então a posição recebe peso normalizado zero", "O vetor de value é zerado diretamente", "Porque os scores são divididos por √d_k"]
    correctIndex: 1
  ...
  - question: "Por que modelos de linguagem causais ainda conseguem treinar todas as posições em paralelo?"
    options: ["Cada posição lê a resposta escondida", "A máscara remove todas as dependências", "A matriz triangular de scores inteira é calculada numa única operação em batch", "O treinamento não usa sequência"]
    correctIndex: 2
