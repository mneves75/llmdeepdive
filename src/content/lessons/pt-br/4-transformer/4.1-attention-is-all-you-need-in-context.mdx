...
quiz:
  - question: "O que o Transformer de 2017 removeu principalmente do seu modelo de sequência?"
    options: ["Parâmetros aprendidos", "Recorrência e convolução", "Embeddings de tokens", "Funções de loss"]
    correctIndex: 1
  ...
  - question: "Por que a cadeia de dependências de uma RNN limita o paralelismo no treinamento, um problema que attention evita?"
    options: ["RNNs não têm parâmetros aprendíveis", "RNNs não podem usar uma função de loss", "Cada estado oculto precisa esperar o estado anterior antes de poder ser calculado", "RNNs exigem vocabulários maiores"]
    correctIndex: 2
  ...
  - question: "No Qwen3.8-27B, o que aconteceu com a pilha uniforme de attention do projeto original?"
    options: ["Attention foi removida por completo", "Toda camada ainda usa attention completa", "Attention foi substituída por convolução", "Só uma camada em quatro mantém attention completa; as outras três usam Gated DeltaNet"]
    correctIndex: 3
...
O custo quadrático foi o preço. Para uma sequência de comprimento $n$, attention convencional forma da ordem de $n^2$ pontuações por pares por head, e esse custo retorna com força quando as janelas de contexto alcançam centenas de milhares de tokens. A trilha 7 trata, em grande parte, de pagar, esquivar ou reestruturar essa conta.
