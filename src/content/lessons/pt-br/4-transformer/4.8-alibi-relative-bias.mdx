...
quiz:
  ...
  - question: "Como o RoPE (lição 4.7) obtém dependência de posição relativa, em contraste com o bias aditivo do ALiBi?"
    options: ["Ele soma um vetor aprendido aos embeddings de token", "Ele rotaciona queries e keys para que o produto escalar dependa da diferença de posição", "Ele multiplica as saídas do softmax pela distância", "Ele ordena as keys por distância antes da attention"]
    correctIndex: 1
  ...
  - question: "Por que usar inclinações diferentes de ALiBi entre os heads?"
    options: ["Para tornar todos os heads idênticos", "Para remover a ordem causal", "Para oferecer várias preferências de distância, do fortemente local ao tolerante a longo alcance", "Para aprender IDs absolutos de token"]
    correctIndex: 2
