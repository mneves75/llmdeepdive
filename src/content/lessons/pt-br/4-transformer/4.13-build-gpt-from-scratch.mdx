...
quiz:
  - question: "Por que a máscara triangular é registrada como buffer?"
    options: ["Ela vira um parâmetro do otimizador", "Ela acompanha o módulo entre devices mas não é treinável", "Ela armazena o texto gerado", "Ela substitui os embeddings de posição"]
    correctIndex: 1
  ...
  - question: "Usando os cinco eixos da lição anterior, como o GPT mínimo é classificado?"
    options: ["Encoder–decoder com cross-attention, denso, híbrido", "Mixture-of-experts decoder-only, multimodal", "Decoder-only, objetivo de próximo token, denso, mixer de attention uniforme, apenas texto", "Encoder-only com objetivo de corrupção"]
    correctIndex: 2
  ...
  - question: "No topo de 27B da escada de parâmetros, qual componente guarda mais parâmetros?"
    options: ["A tabela de embedding de tokens sozinha", "As máscaras causais", "O schedule de learning rate do otimizador", "As FFNs gated, cerca de 17B ao longo das 64 camadas"]
    correctIndex: 3
...
Chame o que ele constrói de **baseline uniforme**: cada camada é o mesmo bloco, o mixer de cada bloco é a mesma attention, as posições vêm de uma única tabela aprendida e a norm é o LayerNorm clássico. Este é o esqueleto pedagógico que a trilha inteira montou — e é exatamente o quadro de que você precisa para apreciar uma instância em produção como o Qwen3.8-27B, que se afasta da uniformidade de propósito em quase toda junta. Primeiro construa a baseline; os desvios vêm no fim.
