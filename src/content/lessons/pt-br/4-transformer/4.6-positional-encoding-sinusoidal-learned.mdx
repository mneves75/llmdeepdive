...
quiz:
  - question: "Que propriedade de ordem falta à self-attention baseada só em conteúdo?"
    options: ["Ela sempre lê apenas o token anterior", "Ela não tem parâmetros aprendidos", "Ela não distingue permutações sem um sinal posicional", "Ela não consegue processar batches"]
    correctIndex: 2
  ...
  - question: "Se vetores de posição são somados aos embeddings, quais tensores de attention acabam carregando informação de posição (lição 4.5)?"
    options: ["Apenas o causal mask", "Apenas a projeção de saída", "Nenhum; a soma é removida antes da attention", "Queries, keys e values, já que todas as projeções de head leem o mesmo residual stream"]
    correctIndex: 3
