...
quiz:
  - question: "No bloco de attention do Qwen3.8-27B, qual é a largura do query state que entra na atenção, e por quê?"
    options: ["5120, porque projeções num Transformer são sempre quadradas", "1024, a mesma da projeção de key", "248.320, o tamanho do vocabulário", "6144, porque 24 heads × head dimension 256 não precisa ser igual ao hidden size 5120 — metade de uma projeção com porta de 12288"]
    correctIndex: 3
  ...
  - question: "Usando o padrão pre-norm da lição anterior, qual expressão descreve a primeira metade de um bloco do Qwen?"
    options: ["x1 = RMSNorm(x + Mixer(x))", "x1 = x + Mixer(RMSNorm(x))", "x1 = Mixer(x) com o stream descartado", "x1 = RMSNorm(x) sozinho"]
    correctIndex: 1
