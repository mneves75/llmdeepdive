...
quiz:
  - question: "O Qwen3.8-27B tem 24 query heads, mas apenas 4 KV heads. Quantos query heads compartilham cada head de key/value?"
    options: ["24, todos eles", "6, já que 24 ÷ 4 = 6", "4", "1, compartilhamento nunca acontece"]
    correctIndex: 1
  ...
  - question: "Num decoder como o Qwen3.8-27B, que restrição vale dentro do cálculo de attention de cada head (lição 4.4)?"
    options: ["Heads podem ler tokens futuros desde que outros não leiam", "Só o primeiro head é mascarado", "O causal mask — as posições de cada head só podem dar attention ao seu prefixo", "A máscara é aplicada depois da projeção de saída"]
    correctIndex: 2
  ...
  - question: "Usar 24 heads garante 24 funções linguísticas distintas?"
    options: ["Sim, por definição arquitetural", "Sim, se causal masking for usado", "Não, porque heads não têm pesos aprendidos", "Não, heads podem ser redundantes ou codificar features distribuídas"]
    correctIndex: 3
